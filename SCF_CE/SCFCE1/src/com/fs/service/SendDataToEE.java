...
public class SendDataToEE {
	...
	/**
     * Builds a JSON payload wrapping all CP entries as a doRecords array.
     * Called from CPOnboardService.releaseData() after mail sending.
     */
    public Map<String, Object> buildCpPayload(String mainRef, String trxRef,
                                               List<Map<String, String>> cpList) {
        Map<String, Object> payload = new HashMap<>();
        payload.put("cMainRef",     mainRef);
        payload.put("cTrxRef",      trxRef);
        payload.put("cUnitCode",    "CSBANK");
        ...
        System.out.println("VMTEST===buildCpPayload | records=" + cpList.size()
                + " | payload=" + payload);
        return payload;
    }

    /**
     * Sends all CP entries extracted from DO_TEMP_DATA to EE's /cpData endpoint
     * in a single API call.
     */
    public String pushCpDataToEE(String mainRef, String trxRef,
                                  List<Map<String, String>> cpList) throws Exception {
        System.out.println("VMTEST===pushCpDataToEE | mainRef=" + mainRef
                + " | cpList size=" + cpList.size());
        ...
        if (response.status >= 200 && response.status < 300) {
            ...
            System.err.println("VMTEST===CP data send FAILED | status=" + response.status
