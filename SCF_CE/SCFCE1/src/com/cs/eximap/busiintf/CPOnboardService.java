...
public Result releaseData(Request req, Component arg1, Result result) throws Exception {
    ...
    if (realOriData != null) {
        ...
        APLog.reportLog("VMTEST===contractRef==" + contractRef + " | mainRef==" + mainRef + " | cntrDocNo==" + cntrDocNo);
        ...
        for (CpEntry cp : cpEntries) {
            APLog.reportLog("VMTEST===Processing CP_ID==" + cp.cpId
                    + " | CP_NAME==" + cp.cpName + " | CP_MAIL==" + cp.cpMail);

            String mailId   = UUID.randomUUID().toString().replace("-", "");
            String sendFrom = mailCfg != null ? mailCfg.get("from").asText() : "support@csbank.com";
            String subject  = mailCfg != null
                    ? mailCfg.get("subject").asText().replace("_", " ")
                    : "You have been invited to onboard";
            String bodyTmpl = mailCfg != null
                    ? mailCfg.get("body").asText()
            ...
            HashMap<String, String> tokenResult   = tokenService.generateRegistrationToken(
                    mainRef, cp.cpId, cp.cpMail, null);
            String inviteLink = tokenResult.get("REGISTRATION_LINK");
            APLog.reportLog("VMTEST===Invite Link==" + inviteLink);

            String mailBody = bodyTmpl
                    .replace("${name}", cp.cpName != null ? cp.cpName : "")
                    .replace("${link}", inviteLink);

            try {
                insertNotifyEmail(mailId, applyBu, applyBy, sendFrom, cp.cpMail, subject, mailBody, trxDs);
                APLog.reportLog("VMTEST===TRX_NOTIFY_EMAIL inserted | mailId==" + mailId + " | to==" + cp.cpMail);
            } catch (Exception dbEx) {
                APLog.reportLog("VMTEST===ERROR TRX_NOTIFY_EMAIL | to==" + cp.cpMail + " : " + dbEx.getMessage());
            }
            ...
            try {
                ...
                APLog.reportLog("VMTEST===ERROR TRX_NOTIFY_EMAIL_REF_TRX | mailId==" + mailId + " : " + dbEx.getMessage());
            }

            try {
                if (smtpCfg != null && mailCfg != null) {
                    sendCpMail(cp.cpMail, mailBody, subject, smtpCfg, sendFrom);
                    APLog.reportLog("VMTEST===Mail SENT to==" + cp.cpMail);
                } else {
                    APLog.reportLog("VMTEST===SKIPPING mail — mail-config.json not loaded for: " + cp.cpMail);
                }
        ...
        // ✅ NEW: Convert CpEntry list → List<Map> and send ALL CP entries to EE in one call
    ...
    private void insertNotifyEmail(
            String mailId, String applyBu, String applyBy,
            String sendFrom, String sendTo, String subject,
            String content, String trxDs) throws Exception {

        // Use SYSDATE for D_INIT_DATE — correct DB date, no type mismatch
        String sql =
            "INSERT INTO CETRX.TRX_NOTIFY_EMAIL " +
            "(C_MAIL_ID, C_BK_GROUP_ID, C_CNTY_CODE, C_MODULE_TYPE, C_MAIL_STATUS, " +
            " C_MSEND_NAME, C_MTMPL_NAME, C_MTEMPL_DESC, D_INIT_DATE, " +
            " C_APPLY_BU, C_APPLY_BY, C_SUBJECT, C_SEND_FROM, C_SEND_TO, " +
            " C_CC, C_CONTENT, C_MAIL_SENDTYPE) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, SYSDATE, ?, ?, ?, ?, ?, ?, ?, ?)";

        APLog.reportLog("VMTEST===TRX_NOTIFY_EMAIL INSERT | mailId==" + mailId + " | to==" + sendTo);

        try (Connection con = DSManager.getConnection(trxDs);
    ...
    private void insertNotifyEmailRefTrx(
            String mailId, String unitCode,
            String mainRef, String eventTimes, String trxRef,
            String trxDs) throws Exception {

        String sql =
            "INSERT INTO CETRX.TRX_NOTIFY_EMAIL_REF_TRX " +
            "(C_MAIL_ID, C_BK_GROUP_ID, C_CNTY_CODE, C_MODULE, " +
            " C_UNIT_CODE, C_MAIN_REF, I_EVENT_TIMES, C_TRX_REF) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    private void sendCpMail(
            String email, String body, String subject,
            JsonNode smtpCfg, String fromAddress) throws Exception {
    ...
    /**
     * Parses DO_TEMP_DATA XML and returns CP_ID + CP_NAME + CP_MAIL
     * for each CP_ADD block found.
     */
        ...
        try {
            ...
            for (int i = 0; i < cpNodes.getLength(); i++) {
                ...
                if (cpMail != null && !cpMail.trim().isEmpty()) {
                	list.add(new CpEntry(cpId, cpName, cpMail.trim(), cntcDetl));
                    APLog.reportLog("VMTEST===Extracted CP_ID==" + cpId
                            + " | CP_NAME==" + cpName + " | CP_MAIL==" + cpMail);
                }
            }
        } catch (Exception e) {
    ...
    private static class CpEntry {
        ...
        CpEntry(String cpId, String cpName, String cpMail,String cntcdetl) {
            this.cpId   = cpId;
            this.cpName = cpName;
            this.cpMail = cpMail;
