...
<xsl:stylesheet exclude-result-prefixes="xalan"
    ...
    <xsl:template match="/">
        <xsl:element name="SCF59">
			...
			<xsl:element name="POBatchNumber"/>
			<xsl:element name="POBatchDate"/>
			<xsl:element name="POBatchCurrency"/>
			<xsl:element name="POTotalAmount"/>
			...
			<xsl:for-each select="/root/PO">
				<xsl:element name="PODetails">
					...
					<xsl:element name="SBRRef">
							<xsl:value-of select="FA_SBR_REF"/>
					</xsl:element>
					<xsl:element name="DocNr">
							<xsl:value-of select="PO_NO"/>
					</xsl:element>
					<xsl:element name="DocDate">
							<xsl:value-of select="PO_DT"/>
					</xsl:element>
					<xsl:element name="DocCcy">
							<xsl:value-of select="PO_CCY"/>
					</xsl:element>
					<xsl:element name="DocAmt">
							<xsl:value-of select="PO_AMT"/>
					</xsl:element>
					<xsl:element name="LatestShipmentDate">
							<xsl:value-of select="FA_LATEST_SHIP_DT"/>
					</xsl:element>
					<xsl:element name="OrderAcknowledgementToIF"/>
					...
					<xsl:element name="POFinBal"/>
					<xsl:element name="POFinId"/>
