    ...
    <xsl:template match="/">
        <xsl:element name="root">
            <xsl:element name="domData">
                <xsl:element name="C_XML_DATA_001">
                    <xsl:value-of select="/message/out-msg-content/C_XML_DATA"/>
                </xsl:element>
                <xsl:element name="TSUR2_TRX_STATUS">
                    <xsl:value-of select="/message/Document/Ack/TxSts/Sts"/>
                </xsl:element>
                <xsl:element name="TSU_MESSAGE_ID">
                    <xsl:value-of select="/message/Document/Ack/AckId/Id"/>
                </xsl:element>
                <xsl:element name="TSU_RL_MSG_ID">
                    <xsl:value-of select="/message/Document/Ack/AckdMsgRef/Id"/>
                </xsl:element>
                <xsl:element name="TSU_RL_MSG_ID_DTTM">
                    <xsl:value-of select="/message/Document/Ack/AckdMsgRef/CreDtTm"/>
                </xsl:element>
                <xsl:element name="TSU_TID">
                    <xsl:value-of select="/message/Document/Ack/TxId/Id"/>
                </xsl:element>
                <xsl:element name="TSU_TRX_DTTM">
                    <xsl:value-of select="/message/Document/Ack/AckId/CreDtTm"/>
                </xsl:element>
            </xsl:element>
        </xsl:element>
    </xsl:template>
