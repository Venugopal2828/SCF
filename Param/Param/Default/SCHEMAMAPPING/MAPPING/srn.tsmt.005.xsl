...
<xsl:stylesheet exclude-result-prefixes="xalan" extension-element-prefixes="base ee cs" version="1.0" xmlns:base="base"
    ...
    <xsl:template match="/">
        <xsl:element name="Document">
            <xsl:element name="AmdmntAccptnc">
                <xsl:element name="AccptncId">
                    <xsl:element name="Id">
                ...
                <xsl:element name="AccptdAmdmntNb">
                    <xsl:element name="Nb">
                        <xsl:value-of select="/root/domData/TSU_AMD_SEQNO"/>
                    </xsl:element>
                </xsl:element>
            </xsl:element>
        </xsl:element>
