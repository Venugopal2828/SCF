...
Mxcvt.pty = function(xmlpath, msg, mtpath, mode) {
  ...
  if (nd.has('IdentifierCode')) {
    ...
    nmaddr = nd.get(nm);
    if (nm.charAt(1) == '5' && nm.charAt(nm.length-1) == 'C') {
      stp.setData(xmlpath + '/Id/OrgId/AnyBIC', nmaddr);
    } else if (nm == 'F50L') {
      stp.setData(xmlpath + '/Nm', nmaddr);;
    }
    return;
  }
...
//        stp.setData(xmlpath + '/Id/PrvtId/Othr/SchmeNm/Cd', 'NIDN');
    ...
    if (cnty && town) {
      if (g_swiftver >= '2025'){
        stp.setData(xmlpath + '/PstlAdr/AdrLine', addr.join(' '));
      } else {
...
Mxcvt.mx2mtParty = function(mtpath, root, mxpty, mxacc, mode ) {
  ...
  var idprefix=''; //"sbza_not_used";
  if (mtpath.indexOf('59') != -1) {
    idprefix='not_used';
  }
  var acc='', bic='';
  var nm='', addr=[];
  ...
  // Agt or Pty
    ...
    var lei = ''+stp.xml(root, mxpty + '/Id/OrgId/LEI'+idprefix, null);
    if (lei != ''){
      // nm += ','+lei;
    }
    ...
    var idOthrPath = '';
    if (stp.has(root, mxpty + '/Id/OrgId/Othr'+idprefix) ){
      idOthrPath=mxpty +"/Id/OrgId/Othr";
    }
    if (stp.has(root, mxpty + '/Id/PrvtId'+idprefix) ){
    ...
    if (idOthrPath != '' ){
      ...
      if (lei != ''){
        if (mtpath.indexOf('50') != -1) {
        optF += '6/'+( idCnty==''? ctry : idCnty ) +'/LEIC/'+lei +'\r\n';
        lei = '';
        } else {
        lei = '/LEIC/'+lei;
        }
 ...
 // stp.log('acc [' + acc + ']  nm:['+nm+'] add [' + addr.join() +'] optF [' +optF);
  ...
  if (tag.startsWith('Choice_')) {
    if (!Mxcvt.isEmpty(bic)) {
     ...
     stp.log("no bic, no name, null; " + mtpath + '  [' + nmaddr + '] [' + nm + '] ' + opt  )
  ...
  if (opt == 'A') {
    ...
    if (tag.startsWith('Choice_50')) {
      stp.setData(mttag , bic);  // 50C bic
    }
  ...
  stp.log(' mx2mtParty END '  + mttag + ' --- ['  + bic + "] " + nmaddr + '  ' + tag);
}
