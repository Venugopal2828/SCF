...
function merge(){
  ...
  var b119 = ''+stp.json(msg, "B3/119", null);
  if (g_swiftver >= '2025'){
    if (mode == 'COVE') {
  ...
  var orgn_mx = '';
  if ('200' === mt) {
    stp.setData("/FICdtTrf/CdtTrfTxInf/PmtId/EndToEndId", b4.get('F20') );  // 'NOTPROVIDED'
    stp.setData("/FICdtTrf/CdtTrfTxInf/Cdtr/FinInstnId/BICFI", stp.getSender() )
  }
  ...
  for (var i = 0; i < lines.size(); i++) {
    ...
    case 'LOCINS':
      if (stp.inExtCode( narr, 'ExternalLocalInstrument1Code')) {
        stp.setData("/FICdtTrf/CdtTrfTxInf/PmtTpInf/LclInstrm/Cd",  narr)
        stp.setData("/FICdtTrf/CdtTrfTxInf/PmtTpInf/LclInstrm/Prtry",  null)
      } else {
        stp.setData("/FICdtTrf/CdtTrfTxInf/PmtTpInf/LclInstrm/Cd",  null)
        stp.setData("/FICdtTrf/CdtTrfTxInf/PmtTpInf/LclInstrm/Prtry",  narr)
      }
      break;
    case 'CATPURP':
      if (stp.inExtCode( narr, 'ExternalCategoryPurpose1Code')) {
        stp.setData("/FICdtTrf/CdtTrfTxInf/PmtTpInf/CtgyPurp/Cd",  narr)
        stp.setData("/FICdtTrf/CdtTrfTxInf/PmtTpInf/CtgyPurp/Prtry",  null)
      } else {
        stp.setData("/FICdtTrf/CdtTrfTxInf/PmtTpInf/CtgyPurp/Cd",  null)
    ...
    case 'PURP':
      ...
      stp.setData("/FICdtTrf/CdtTrfTxInf/Purp/Prtry",  null)
      } else {
      stp.setData("/FICdtTrf/CdtTrfTxInf/Purp/Cd",  null)
