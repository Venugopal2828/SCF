...
//  if ('STP' == ''+stp.json(msg, "B3/119", null)){
//    if (g_swiftver >= '2025'){
  ...
  ChrgsInf(true);
	...
	for (var i = 0; i < lines.size(); i++) {
	  ...
	  case 'SVCLVL':
	    ...
	    break;
	  case 'LOCINS':
        if (stp.inExtCode( narr, 'ExternalLocalInstrument1Code')) {
        stp.setData("/FIToFICstmrCdtTrf/CdtTrfTxInf/PmtTpInf/LclInstrm/Cd",  narr)
        stp.setData("/FIToFICstmrCdtTrf/CdtTrfTxInf/PmtTpInf/LclInstrm/Prtry",  null)
        } else {
        stp.setData("/FIToFICstmrCdtTrf/CdtTrfTxInf/PmtTpInf/LclInstrm/Cd",  null)
        stp.setData("/FIToFICstmrCdtTrf/CdtTrfTxInf/PmtTpInf/LclInstrm/Prtry",  narr)
        }
	    break;
	  case 'CATPURP':
        if (stp.inExtCode( narr, 'ExternalCategoryPurpose1Code')) {
        stp.setData("/FIToFICstmrCdtTrf/CdtTrfTxInf/PmtTpInf/CtgyPurp/Cd",  narr)
        stp.setData("/FIToFICstmrCdtTrf/CdtTrfTxInf/PmtTpInf/CtgyPurp/Prtry",  null)
        } else {
        stp.setData("/FIToFICstmrCdtTrf/CdtTrfTxInf/PmtTpInf/CtgyPurp/Cd",  null)
	...
	stp.log("size: " + infoNxtAgt.length);
	while (infoNxtAgt.length > 6) {
	  infoNxtAgt.pop();
...
// stp.fmt( "15d")
  ...
  if (!stp.has(root, 'CdtTrfTxInf/InstdAmt')){
    // avoid D49
    ...
    var sender = "" + stp.xml(hdr, "Fr/FIId/FinInstnId/BICFI");
    var receiver = "" + stp.xml(hdr, "To/FIId/FinInstnId/BICFI");
    var sender_cnty = sender.substring(4, 6);
    var receiver_cnty = receiver.substring(4, 6);
    stp.log ('D49'+ receiver  + " " + receiver_cnty  + d49_cc.indexOf(sender_cnty )    + " " + d49_cc.indexOf(receiver_cnty )  );
stp.log ('D49'+  (d49_cc.indexOf(sender_cnty )!=-1 && d49_cc.indexOf(receiver_cnty) != -1)  )
    if (d49_cc.indexOf(sender_cnty )!=-1 && d49_cc.indexOf(receiver_cnty) != -1) {

    stp.log ('D49'+ stp.getSender()  + " " + d49_cc.indexOf(sender_cnty )  );
    var f33Amt = ''+stp.xml(root, "CdtTrfTxInf/IntrBkSttlmAmt");
    var f33Ccy = ''+stp.xml(root, 'CdtTrfTxInf/IntrBkSttlmAmt/@Ccy');
    stp.setData("TEXT/F33B/Amount", f33Amt )
    stp.setData("TEXT/F33B/Currency", f33Ccy  )
    }
  }
...
function ClrChanl(isMerge){
	...
	for (var i = 0; i < parties.length; i++) {
        ...
        var nm = "PartyIdentifier";
        var obj = ''+stp.json(nd, nm, null);
        if (obj!=''){
            nm = ''+nd.get("@nm");
            if (nm.charAt(nm.length-1) == 'C') {
              obj = ''+nd.get(nm);
            }
        }
        stp.log('clr chancel '+obj + ' \n'+ nd)
        var s = ''+obj;
