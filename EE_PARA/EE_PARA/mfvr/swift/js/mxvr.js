...
Mx.chk_nonError = function(left,right,exist,parent,error){
  if(ExSvr.exist(exist, parent)){
    if (!ExSvr.exist(left, parent) && !ExSvr.exist(right, parent)) {
      ExSvr.debug( ' E  ' + left + '  ' + right + '  - ' + ExSvr.exist(left, parent));
...
Mx.chk_Slash = function(value,error, index){
  ...
  if (index > 10 ) {
    if (value.charAt(0)=='/' || ( value.length>=index && value.charAt(index-1)=='/'  ) ){
      ExSvr.add_err(error, 'slash error:'+value);
      return;
    }
    if (value.length>=index ) value = value.substring(0,index);
  } else {
...
Mx.chk_TwnNmAndCtry = function(path,parent){
  if(ExSvr.exist(path,parent)){
    ...
    // ExSvr.debug(' vvv ' + Ex.mfvr.ver + '  ' + PstlAdr.length + "  " + path + "  "  );
    if (Ex.mfvr.ver < '2025') {
    ...
    if(ExSvr.exist('AdrLine',PstlAdr[0])){
      if (Ex.mfvr.ver < '2025')  return;
      var len0 = ExSvr.getflds('*', PstlAdr[0]).length;
      var lines = ExSvr.getflds('AdrLine', PstlAdr[0]);
      var len1 = lines.length;
      if (len0 == len1) {
        var err = false;
        for(var i=0;i<lines.length;i++){
          var line = ''+ExSvr.get_val(null, lines[i]);
          if (line.length > 35) {
            err = true;
          }
        }
        if (err) {
          ...
          ExSvr.add_err("CBPR_GracePeriod_Unstructured_FormalRule",pp+path);
        }
      } else {
        if (!ExSvr.exist('Ctry',PstlAdr[0])||!ExSvr.exist('TwnNm',PstlAdr[0]) || len1 > 2) {
        var pp = parent ? ExSvr.get_path(parent)+'/' : '';
        ExSvr.add_err("CBPR_GracePeriod_Hybrid_FormalRule",pp+path);
        }
      }
    } else {
      if(!ExSvr.exist('Ctry',PstlAdr[0])||!ExSvr.exist('TwnNm',PstlAdr[0])){
        var pp = parent ? ExSvr.get_path(parent)+'/' : '';
        var errcode= Ex.mfvr.ver < '2025'? "CBPR_Town_Name_And_Country_FormalRule" : "CBPR_GracePeriod_Structured_FormalRule";
        ExSvr.add_err(errcode,pp+path+' AdrLine absent,Ctry and TwnNm must exist!');
      }
    }
  }
}
...
Mx.chk_DCBICFI=function(path,parent,pattern,error){
  if(ExSvr.exist(path+'/CdtrAgt/FinInstnId/BICFI')&&ExSvr.exist(path+'/DbtrAgt/FinInstnId/BICFI')){
    ...
    if(reg.test(CdtrAgtBICFI)&&reg.test(DbtrAgtBICFI)){
      var ary = ['/Dbtr/Nm','/DbtrAcct/Id/IBAN', '/Cdtr/Nm','/CdtrAcct/Id/IBAN' ];
      for (i in ary) {
       if (!ExSvr.exist(path+ary[i]) ) {
          ExSvr.add_err(error, path+ary[i] +' must exist');
       }
      }
    }
  }
