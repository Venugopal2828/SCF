...
function CPYT_DR_AMT_DRCCY() {
    try {
        var drBuyRate; // Utility Auto Fix Comments
        drBuyRate = document.MAINFORM.CPYT_DR_BUY_RATE.value;
        if (drBuyRate != "" && drBuyRate != null) {
            amt = SYS_FloatMul(SYS_BeFloat(document.MAINFORM.CPYT_DR_AMT_TXCCY.value),SYS_BeFloat(drBuyRate));
            document.MAINFORM.CPYT_DR_AMT_DRCCY.value = SYT_AmtFormat(document.MAINFORM.CPYT_DR_CCY.value, amt);
        } else {
...
function CPYT_DR_AMT_DRCCY_DRRATE() {
    try {
...
function CPYT_DR_AMT_TXCCY() {
    try {
        ...
        document.MAINFORM.CPYT_DR_AMT_TXCCY.value = SYS_FloatMul(SYS_BeFloat(document.MAINFORM.CPYT_DR_PER.value),SYS_BeFloat(amt)) / 100;
...
function CPYT_DR_AMT_TXCCYfromDrccy() {
    try {
        var amt; // Utility Auto Fix Comments
        var ccy; // Utility Auto Fix Comments
    ...
    try {
        ...
        if (document.MAINFORM.CPYT_DR_ID.value != "") {
            ...
            if (sAccType == "CUSTOMER") {
                ...
                SYS_GetTableDataByRule('Debit_CPYT_DR_ID', '1');
            } else if (sAccType == "NOSTRO" || sAccType == "VOSTRO") {
                //SYS_GetCUBK('Debit_CPYT_DR_ID_BK', 'CPYT_DR_ID');
                SYS_GetTableDataByRule('Debit_CPYT_DR_ID_BK', '1');
            }
        } else {
...
function PaymentDebit_CancelCheck() {
    try {
        return true;
    } catch (e) {
        DisExcpt("SSSS_PaymentDebit.js", e);
...
function PaymentDebit_ConfirmBusinessCall() {
    try {
...
function PaymentDebit_ConfirmBusinessCheck() {
    try {
...
function PaymentDebit_ConfirmBusinessCheckSave() {
    try {
        return true;
    } catch (e) {
        DisExcpt("SSSS_PaymentDebit.js", e);
...
function DebitCheckRecordStatus() {
    try {
        var targetDO; // Utility Auto Fix Comments
...
function DebitRemoveSelectOption(selectObj, optionValue) {
    try {
...
function Debit_Cal_CPYT_DR_AC_TYPE() {
    try {
        ...
        if (SYS_MODULE_NAME == "IMCO") {
            ...
            if (fromccy == SYS_LOCAL_CCY) {
                if (toccy != SYS_LOCAL_CCY) {
                    ...
                    cpytDebitAmt = SYS_FloatMulToString(trxDrTTL, drAmtPer);
                    if (SYS_BeFloat(cpytDebitAmt) <= 50000) {
                        SYS_GetExchangeRate_S(fromccy, toccy, 'Buying Rate', 'CPYT_DR_BUY_RATE'); //Buying Rate
                    } else {
                        SYS_GetExchangeRate_S(fromccy, toccy, 'Selling Rate', 'CPYT_DR_BUY_RATE');
                    }
                } else {
                ...
                if (toccy == SYS_LOCAL_CCY) {
                    ...
                    vAmt = SYS_FloatMulToString(trxDrTTL, drAmtPer);
                    if (SYS_BeFloat(vAmt) <= 0) {
                        cpytDebitAmt = 0;
                    } else {
                        //SYS_calExchAmt(fromccy, SYS_LOCAL_CCY, trxDrTTL * drAmtPer, 'Booking Rate', 'cpytDebitAmt');
                        SYS_calExchAmt(fromccy, SYS_LOCAL_CCY, SYS_FloatMulToString(trxDrTTL, drAmtPer), 'Booking Rate', 'cpytDebitAmt');
                    }
                    if (SYS_BeFloat(cpytDebitAmt) <= 50000) {
                        SYS_GetExchangeRate_S(fromccy, toccy, 'Selling Rate', 'CPYT_DR_BUY_RATE'); //Buying Rate
                    } else {
                        SYS_GetExchangeRate_S(fromccy, toccy, 'Buying Rate', 'CPYT_DR_BUY_RATE');
                    }
                } else {
...
function PaymentDebit_InitValues() {
    try {
...
function PaymentDebit_PostconditionOnInit() {
    try {
        var ccy;
        var discountflag;
        var func_nm;
...
function PaymentDebit_PreconditionOnInit() {
    try {
...
function Set_CPYT_DR_ID_from_MAIN() {
    try {
        if (SYS_MODULE_NAME == "EPLC") {
...
function PaymentDebit_initFieldEvent() {
    try {
        document.MAINFORM.CPYT_DR_AC.onchange = CPYT_DR_AC_onchange;
        document.MAINFORM.CPYT_DR_AC_BTN.onclick = CPYT_DR_AC_BTN_onclick;
        document.MAINFORM.CPYT_DR_AC_OWNER_BTN.onclick = CPYT_DR_AC_OWNER_BTN_onclick;
        document.MAINFORM.CPYT_DR_AC_TYPE.onchange = CPYT_DR_AC_TYPE_onchange;
        document.MAINFORM.CPYT_DR_AMT_DRCCY.onchange = CPYT_DR_AMT_DRCCY_onchange;
        document.MAINFORM.CPYT_DR_AMT_TXCCY.onchange = CPYT_DR_AMT_TXCCY_onchange;
        document.MAINFORM.CPYT_DR_BUY_RATE.onchange = CPYT_DR_BUY_RATE_onchange;
        document.MAINFORM.CPYT_DR_CCY.onchange = CPYT_DR_CCY_onchange;
        document.MAINFORM.CPYT_DR_ID.onchange = CPYT_DR_ID_onchange;
        document.MAINFORM.CPYT_DR_PER.onchange = CPYT_DR_PER_onchange;
        document.MAINFORM.CPYT_DR_RATE.onchange = CPYT_DR_RATE_onchange;
        document.MAINFORM.CPYT_DR_VAL_DATE.onchange = CPYT_DR_VAL_DATE_onchange;
    } catch (e) {
        DisExcpt("SSSS_PaymentDebit.js", e);
    }
}

function CPYT_DR_AC_onchange() {
    try {
        Get_CPYT_DR_AC();
        Debit_ExchangingRate();
    } catch (e) {
...
function CPYT_DR_AC_BTN_onclick() {
    try {
...
function CPYT_DR_AC_OWNER_BTN_onclick() {
    try {
...
function CPYT_DR_AC_TYPE_onchange() {
    try {
...
function CPYT_DR_AMT_DRCCY_onchange() {
    try {
        AmtFromatAmtDrccy();
...
function CPYT_DR_AMT_TXCCY_onchange() {
    try {
        CPYT_DR_PER();
        CPYT_DR_AMT_DRCCY();
        AmtFormatAmtTrxccy();
...
function CPYT_DR_BUY_RATE_onchange() {
    try {
        CPYT_DR_AMT_DRCCY();
        AmtFromatAmtDrccy();
    } catch (e) {
...
function CPYT_DR_RATE_onchange() {
    try {
...
function CPYT_DR_VAL_DATE_onchange() {
    try {
