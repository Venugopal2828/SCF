"path:SCRN/o2m/FAEF_FinancingReturnLib_PO.jsp";

var csDOScreenProto = Object.create(csDOScreenBaseProto || {});
var FA_SETTLE_FLG= SYS_getValueFromMain("FA_SETTLE_FLG");

csDOScreenProto.CAL_PENALTY_INT_AMT = function() {
    try {
        if (document.MAINFORM.FA_PEN_INT_SDT.value != '' && document.MAINFORM.FA_PEN_INT_EDT.value != '' && SYS_BeFloat(document.MAINFORM.FA_PEN_INT_RT.value) > 0) {
            var intdays = SYS_GetSubDays(document.MAINFORM.FA_PEN_INT_SDT.name, document.MAINFORM.FA_PEN_INT_EDT.name);
            document.MAINFORM.FA_PEN_INT_AMT.value = (SYS_BeFloat(document.MAINFORM.PO_LOAN_BAL.value) * SYS_BeFloat(document.MAINFORM.FA_PEN_INT_RT.value) * 1000 / (SYS_BeFloat(document.MAINFORM.IA_I_BASE_DAYS.value) * 1000)) * intdays;
            document.MAINFORM.FA_PEN_INT_AMT.value = SYT_CCY_AMT(document.MAINFORM.PO_CCY.value, document.MAINFORM.FA_PEN_INT_AMT.value);
            document.MAINFORM.FA_PEN_INT_AMT.fireEvent('onchange');
        }
    } catch (e) {
...
csDOScreenProto.CHECK_PMT_AMT = function() {
    try {
        ...
        if (FA_SETTLE_FLG =='Loan' && TEMP_AMT10 < PMT_AMT) {
...
csDOScreenProto.Cal_INV_LOAN_BAL = function() {
    try {
        ...
        TEMP_AMT10 = SYS_BeFloat(document.MAINFORM.FA_TEMP_AMT10.value);
        CPYT_STL_AMT = SYS_BeFloat(document.MAINFORM.CPYT_STL_AMT.value);
        //TSU_TTL_NET_AMT = SYS_BeFloat(document.MAINFORM.TSU_TTL_NET_AMT.value);
        if (PMT_AMT == '') {
            document.MAINFORM.PO_LOAN_BAL.value = TEMP_AMT10;
           // document.MAINFORM.CPYT_STL_AMT.value = TSU_TTL_NET_AMT;
        } else {
            document.MAINFORM.PO_LOAN_BAL.value = TEMP_AMT10 - SYS_BeFloat(PMT_AMT);
            if(CPYT_STL_AMT > 0){
            document.MAINFORM.TSU_TTL_NET_AMT.value = document.MAINFORM.CPYT_STL_AMT.value;
            document.MAINFORM.CPYT_STL_AMT.value = SYT_CCY_AMT(document.MAINFORM.PO_CCY.value,SYS_BeFloat(document.MAINFORM.CPYT_STL_AMT.value) - SYS_BeFloat(PMT_AMT));
          }

        }
        document.MAINFORM.PO_LOAN_BAL.value = SYT_CCY_AMT(document.MAINFORM.PO_CCY.value, document.MAINFORM.PO_LOAN_BAL.value);
    } catch (e) {
        DisExcpt("SSSS_FAEF_FinancingReturnLib_PO.js", e);
    }
}
...
csDOScreenProto.ConfirmBusinessCall = function() {
    try {
        ...
        _do = SYS_getDoByXpath('POReturnFin.Settle_loan_po'); // Utility Auto Fix Comments
        ...
        document.MAINFORM.FA_PAID_PRIN_AMT.value =document.MAINFORM.FA_PMT_AMT.value;
        PO_STATUS();
    } catch (e) {
        DisExcpt("SSSS_FAEF_FinancingReturnLib_PO.js", e);
    }
...
csDOScreenProto.ConfirmBusinessCheck = function() {
    try {
        ...
        if (recs.length > 0) {
            ...
            if ((overintamt == 0 && intchgtype == '1' && subday < 0) || (refundintamt == 0 && intchgtype == '1' && subday > 0)||(intamtcheck == 0 && intchgtype == '2')) {
...
csDOScreenProto.LoadDODataOnInit = function() {
    try {
        ...
        var recs; // Utility Auto Fix Comments
        node = SYS_getDoByXpath('POReturnFin.Settle_loan_po');
        recs = SYS_getRecords(node);
        if (recs.length <= 0 && SYS_FUNCTION_TYPE == 'PM') {
            SYS_GetDataForDO_S('SUBPOReturnFin', 'A', false, '', "Settle_loan_po");
        }
        Refesh_Loan_do();
        Init_Loan_DO();
        if (recs.length > 0) {
...
csDOScreenProto.MPO_PEN_INT_Fields = function() {
    try {
        if (document.MAINFORM.FA_PEN_INT_SDT.value == '' || SYS_BeFloat(document.MAINFORM.FA_PEN_INT_RT.value) == 0) {
...
csDOScreenProto.POReturnFin_Settle_loan_po_FA_LOAN_PAID_AMT_onChange = function(ed, record, index, name) {
    try {
        ...
        temp_acc_amt = document.MAINFORM.CPYT_STL_AMT.value;
...
if (intChgType == '1' && pmtValDt < loanDueDt) {
    ...
    record.data['PO_LOAN_EBAL'] = loanbal - v;
    ...
    if (v > da) {
        ...
        record.data['PO_LOAN_EBAL'] = loanbal - (v - da);
        forReturnAmt(vindex, vname);
        return v;
    } else {
        ...
        if (v > da) {
            record.data['FA_LOAN_IPAID_AMT'] = da;
            record.data['FA_TEMP_AMT19'] = oamt;
            if((SYS_BeFloat(v) -SYS_BeFloat(temp_acc_amt)) <0){
            	alert('Paid loan amount cannot be less than Invoices Loan Amount to Temporary Account');
            	 record.data['FA_LOAN_PPAID_AMT'] = 0;
                record.data['FA_LOAN_IPAID_AMT'] = 0;
                record.data['FA_TEMP_AMT19'] = 0;
                record.data['PO_LOAN_EBAL'] = loanbal * 1000 / 1000;
        ...
        var node = SYS_getDoByXpath('POReturnFin.Settle_loan_po');
...
csDOScreenProto.PO_STATUS = function() {
    try {
        var po_max_loan = SYS_getValueFromMain('PO_MAX_LOAN_PERC');
        var po_loan_avl = SYS_BeFloat(document.MAINFORM.PO_AMT.value) * SYS_BeFloat(po_max_loan) / 100 - SYS_BeFloat(document.MAINFORM.FA_FIN_LOAN_AMT.value);
        var po_loan_bal = SYS_BeFloat(document.MAINFORM.PO_LOAN_BAL.value);
        var po_bal=SYS_BeFloat(document.MAINFORM.PO_BAL.value);
...
csDOScreenProto.PostconditionOnInit = function() {
    try {
        document.MAINFORM.PO_CCY.value = SYS_getValueFromMain('FA_DOC_CCY');
        ...
        document.MAINFORM.PO_LOAN_ID.value = SYS_getValueFromMain('FA_LOAN_ID');
         document.MAINFORM.TEMP_LOAN_ID.value = SYS_getValueFromMain('FA_LOAN_ID');
        geteLOANBaseDay();
        ...
        document.MAINFORM.FA_OVD_INT_AMT.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, document.MAINFORM.FA_OVD_INT_AMT.value);
            if (FA_SETTLE_FLG !='Loan'){
            	document.MAINFORM.FA_PMT_AMT.value =SYT_AmtFormat(document.MAINFORM.PO_CCY.value, document.MAINFORM.PO_AMT.value);
            }
        document.MAINFORM.FA_PMT_AMT.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, document.MAINFORM.FA_PMT_AMT.value);
        //document.MAINFORM.FA_PEN_INT_AMT.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, document.MAINFORM.FA_PEN_INT_AMT.value);
        document.MAINFORM.FA_FIN_LOAN_AMT.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, document.MAINFORM.FA_FIN_LOAN_AMT.value);
        document.MAINFORM.FA_LOAN_INT_AMT.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, document.MAINFORM.FA_LOAN_INT_AMT.value);
        //MPO_PEN_INT_Fields();
...
csDOScreenProto.dcmAdd = function(arg1, arg2) {
    try {
...
csDOScreenProto.forReturnAmt = function(index, name) {
    try {
        var PO_LOAN_EBAL; // Utility Auto Fix Comments
        ...
        if (document.MAINFORM.FA_INT_CHG_TYPE.value == '2') {
            ...
            document.MAINFORM.FA_LOAN_INT_AMT.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, FA_LOAN_INT_AMT);
        }
        //alert('sum:'+_value);
        document.MAINFORM.FA_PMT_AMT.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, FA_PMT_AMT);
        EEHtml.fireEvent(document.MAINFORM.FA_PMT_AMT, 'onchange');
        //alert('Pmtvalue'+FA_PMT_AMT);

        document.MAINFORM.FA_OVD_INT_AMT.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, FA_OVD_INT_AMT);
        PO_LOAN_EBAL = SYS_getFieldSumValue(_do, "PO_LOAN_EBAL", 2); // Utility Auto Fix Comments
        document.MAINFORM.PO_LOAN_BAL.value = SYT_AmtFormat(document.MAINFORM.PO_CCY.value, PO_LOAN_EBAL);
        EEHtml.fireEvent(document.MAINFORM.PO_LOAN_BAL, 'onchange');
    } catch (e) {
        DisExcpt("SSSS_FAEF_FinancingReturnLib_PO.js", e);
    }
}
