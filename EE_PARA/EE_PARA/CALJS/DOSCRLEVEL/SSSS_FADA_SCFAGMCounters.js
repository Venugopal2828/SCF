"path:SCRN/DO/FADA_SCFAGMCounters.jsp";
...
csDOScreenProto.ConfirmBusinessCheck = function() {
    try {
        ...
        // ✅ If SELF ENROL = YES → skip all validations
        if (selfEnrol === "YES") {
            return true;
        }*/
    ...
    try {
        ...
        if ("E" == state) {
            ...
            if (document.MAINFORM.FA_TEMP5.value == null || document.MAINFORM.FA_TEMP5.value == '' || document.MAINFORM.FA_TEMP5.value == 'null') {
                ...
                SYT_ChangeFldClass(document.MAINFORM.FA_LMT_DUE_DT, 'O');
...
csDOScreenProto.CSREF = function(ref) {
    try {
        var UnitCode;
        var month;
        var pre;
        ...
        year = year.substr(2, 2);
        month = month.substr(5, 2);
		document.MAINFORM.FA_COUNTER_ID.value = "C" + year + month + ref;
		...
		if (selfEnrol === "YES") {
			...
			document.MAINFORM.FA_COUNTER_ID.value = "";
		}*/
			
    } catch (e) {
        DisExcpt("SSSS_FADA_SCFAGMCounters.js*CSREF", e);
    }
}
        ...
        if (selfEnrol === "YES") {
            ...
            clearFields.forEach(function (fld) {
                if (form[fld]) {
                    ...
                    form[fld].value = "";
                    form[fld].closest("tr").style.display = "none";
                     SYT_ChangeFldClass(form[fld], 'P'); // BEST WAY
                }
            });
           ...
           clearFields.forEach(function (fld) {
                if (form[fld]) {
                    //SYT_ChangeFldClass(form[fld], 'O');
                    form[fld].closest("tr").style.display = "";
                     SYT_ChangeFldClass(form[fld], 'O');
                     SYS_changeClassName('button1', 'M');
                     SYS_changeClassName('button2', 'M');
                     SYT_ChangeFldClass(form.FA_COUNTER_NM, 'M');
                     SYT_ChangeFldClass(form.FA_COUNTER_REG_NO, 'M');
                     SYT_ChangeFldClass(form.FA_LMT_AMT, 'M');
                     SYT_ChangeFldClass(form.FA_COUNTER_ACC, 'M');
                     SYT_ChangeFldClass(form.FA_CUST_FLAG, 'M');
                     SYT_ChangeFldClass(form.FA_COUNTER_CE_FLG, 'M');
                     SYT_ChangeFldClass(form.FA_LMT_DUE_DT, 'M');
                     SYT_ChangeFldClass(form.FA_COUNTER_CONT_EM, 'M');
...
csDOScreenProto.SELF_ENROL_onchange = function (event) {
    try {
        ...
        if (selfEnrol === "YES") {
            ...
            SYT_ChangeFldClass(form.FA_COUNTER_CONT_EM, 'M');
            SYT_ChangeFldClass(form.FA_COUNTER_NM, 'M');
			SYT_ChangeFldClass(form.FA_COUNTER_ID, 'P');
			...
			if (form.FA_LMT_DUE_DT) {
				form.FA_LMT_DUE_DT.closest("tr").style.display = "";
				SYT_ChangeFldClass(form.FA_LMT_DUE_DT, 'O');
			}

            // Make FA_COUNTER_REG_NO mandatory and visible
            if (form.FA_COUNTER_ID) {
                form.FA_COUNTER_ID.closest("tr").style.display = "";
                SYT_ChangeFldClass(form.FA_COUNTER_ID, 'P');
            }

            // Assign values from main after clear
            form.FA_BUSI_TYPE.value = SYS_getValueFromMain("FA_BUSI_TYPE");
            form.FA_ANCHOR_ID.value = SYS_getValueFromMain("FA_ANCHOR_ID");
            form.FA_LMT_CCY.value = SYS_getValueFromMain("FA_ANCHOR_CCY");
            form.FA_LMT_AMT.value = SYS_getValueFromMain("FA_ANCHOR_AMT");
            form.FA_LMT_AMT.value = SYT_CCY_AMT(form.FA_LMT_CCY.value, form.FA_LMT_AMT.value);
            form.FA_LMT_VAL_DT.value = SYS_getValueFromMain("FA_AGM_VAL_DT");
            form.FA_LMT_DUE_DT.value = SYS_getValueFromMain("FA_AGM_DUE_DT");
            form.C_MAIN_REF.value = SYS_getValueFromMain("C_MAIN_REF");
            form.FA_SERVICE_REQ.value = SYS_getValueFromMain("FA_SERVICE_REQ");
            SYS_GetSubPageRefNo_S('FADA_COUNTER_CSREF', CSREF, "", "CSREF", "CSREF");

        }else {
				if (form.FA_COUNTER_ID) {
					form.FA_COUNTER_ID.value = "";
                } 
...
csDOScreenProto.FA_COUNTER_NM_onchange = function(event) {
    try {
        var field = document.MAINFORM.FA_COUNTER_NM;
        var value = field.value.replace(/\s+/g, " ").trim();
		field.value = value;

        if (value === "") {
            alert("Counter Name cannot be blank");
            field.focus();
            return false;
        }
       var letters = /^[A-Za-z]+([. ]?[A-Za-z]+)*$/;
       if (!letters.test(value)) {
            alert("Only alphabets and dot (.) are allowed in Counter Name field.");
            field.value = '';
            field.focus();
            return false;
        }
        ...
        DisExcpt("SSSS_FADA_SCFAGMCounters.js*FA_COUNTER_NM_onchange", e);
    }
}