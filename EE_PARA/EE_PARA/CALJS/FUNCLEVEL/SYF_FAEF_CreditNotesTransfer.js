...
csFuncLevelProto.SYF_FAEF_Get_EXCH_RT6 = function() {
    try {
        ...
        /*
SYS_GetExchangeRate_S(document.MAINFORM.FA_DOC_CCY.value, 'RON', 'Booking Rate', 'EXCH_RT4');
exRt4=SYS_BeFloat(document.MAINFORM.EXCH_RT4.value);
SYS_GetExchangeRate_S(document.MAINFORM.FA_LMT_CCY.value, 'RON', 'Booking Rate', 'EXCH_RT5');
exRt5=SYS_BeFloat(document.MAINFORM.EXCH_RT5.value);
rate=exRt4/exRt5;
//rate =SYS_BeFloat(document.MAINFORM.EXCH_RT4.value)/SYS_BeFloat(document.MAINFORM.EXCH_RT5.value);
document.MAINFORM.EXCH_RT6.value = roundUp(rate, 6);
document.MAINFORM.EXCH_RT6.fireEvent('onchange');
*/
        ...
        DisExcpt("SYF_FAEF_CreditNotesTransfer.js", e);
    }
}
...
csFuncLevelProto.SYF_FAEF_Chk_FinanceReturnforDocument = function() {
    try {
        ...
        _do = SYS_getDoByXpath('CreNote'); // Utility Auto Fix Comments
...
csFuncLevelProto.SYF_FAEF_Cal_cableChargeCCYReload = function() {
    try {
        ...
        //FA_CB_FEE_CCY.options.add(new Option(SYS_LOCAL_CCY,SYS_LOCAL_CCY));
        FA_CB_FEE_CCY.options.add(new Option('USD', 'USD'));
        //FA_CB_FEE_CCY.selectedIndex = FA_CB_FEE_CCY.options.length-1;
...
csFuncLevelProto.SYF_FAEF_Cal_FA_INV_CCY = function() {
    try {
        ...
        SYS_refreshChildDoValue("CreNote", 'FA_DOC_CCY', ccy);
    } catch (e) {
        DisExcpt("SYF_FAEF_CreditNotesTransfer.js", e);
    }
}
...
csFuncLevelProto.SYF_FAEF_SetRefNo = function(ref) {
    try {
        ...
        sub = 'CNT';
        document.MAINFORM.FA_TRF_REF.value = pre + UnitCode + year + month + ref + sub;
...
csFuncLevelProto.LoadDODataOnInit = function() {
	...
	if('D'==status){
		num=SYS_getcurrRecordCount("CreNote");
		if(num==0){
			Cal_forControlDocCcy('2');
		}else{
			Cal_forControlDocCcy('1');
     		}
	}else{
		if(num==0){
			Cal_forControlDocCcy('2');
...
csFuncLevelProto.PreconditionOnInit = function() {
    try {
        ...
        //SYT_loadExchRate();
        SYS_GetRefNo('FAEF_INV_TRF', 'SYF_FAEF_SetRefNo', '', '', '', 'FA_TRF_REF');
    } catch (e) {
        DisExcpt("SYF_FAEF_CreditNotesTransfer.js", e);
    }
}
...
csFuncLevelProto.ConfirmBusinessCheck = function() {
    try {
        ...
        //SYM_FAEF_forBAFields(document.MAINFORM.FA_BA_LINK.name);
        if (!SYT_checkFactoringChildRecord('CreNote')) {
            return false;
        }
...
csFuncLevelProto.PostconditionOnInit = function() {
    try {
        ...
        //SYM_FAEF_setDocCCY(); ByMaria20080929
        SYT_RELE_CREA_BY();
...
csFuncLevelProto.SYF_FAEF_Cal_forChildtoMainScreen = function(node, recordId, status) {
    try {
        ...
        if ('D' != status) {
            ...
            newfinRetsum = SYS_BeFloat(finRetsum);
    ...
    try {
        ...
        if (num > 0) {
            ...
            for (i = 0, len = arrayvalue.length; i < len; i++) {
                ...
                /*
	if(SYS_BeFloat(recordTypeTemp)!=paySamt&&SYS_BeFloat(recordTypeTemp)!=payBamt){
...
csFuncLevelProto.FLD_FAEF_FA_EF_HAN_CHG_PAMT_onchange = function(event) {
    try {
        ...
        SYS_refreshChildDoValue("CreNote", 'FA_EF_HAN_CHG_PAMT', document.MAINFORM.FA_EF_HAN_CHG_PAMT.value);
...
csFuncLevelProto.FLD_FAEF_FA_IF_HAN_CHG_PAMT_onchange = function(event) {
    try {
        ...
        if (document.MAINFORM.FA_DOC_CCY.value == document.MAINFORM.FINC_CCY1.value) {
            ...
            SYS_refreshChildDoValue("CreNote", 'FA_IF_HAN_CHG_PAMT', document.MAINFORM.FA_IF_HAN_CHG_PAMT.value);
            SYF_FAEF_Cal_forGridtoMainScreen();
        }
    } catch (e) {
        DisExcpt("SYF_FAEF_CreditNotesTransfer.js", e);
