...
csFuncLevelProto.SYF_IMCO_CAL_LOCAL_Charge = function() {
    try {
        ...
        DisExcpt("SYF_IMCO_PaymentDP.js", e);
    }
}
...
csFuncLevelProto.ConfirmBusinessCall = function() {
    try {
        ...
        document.MAINFORM.LOCAL_AMT.value = document.MAINFORM.COLL_TRX_CCY_BAL.value;
...
csFuncLevelProto.PostconditionOnInit = function() {
    try {
        ...
        if (SYS_FUNCTION_TYPE == 'RE' || SYS_FUNCTION_TYPE == 'EC' || SYS_FUNCTION_TYPE == 'IQ') {
            document.MAINFORM.LOCAL_AMT.value = SYT_AmtFormat(document.MAINFORM.LOCAL_CCY.value, document.MAINFORM.LOCAL_AMT.value);
            SYF_IMCO_CHK_SEPARATE_FLG();
            CHG_TotalAmountOnChange(); //STP 215
        }
...
csFuncLevelProto.ConfirmBusinessCheck = function() {
    try {
        ...
        if (nBILL_AMT_FM_DRWE > nCOLL_TRX_CCY_AMT) {
            alert("Bill Amount can not greater than Collection Amount")
            return false;
        }
        return true;
    } catch (e) {
...
csFuncLevelProto.SYF_IMCO_CHG_PMT_DT = function() {
    try {
