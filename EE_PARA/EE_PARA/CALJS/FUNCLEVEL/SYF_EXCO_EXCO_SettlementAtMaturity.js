...
csFuncLevelProto.SYF_EXCO_loadDoDataComplete = function() {
    try {
        ...
        EEHtml.fireEvent(document.MAINFORM.DRWNG_AMT_COLL_CCY, "onchange"); //added Jax 2020/5/12
    } catch (e) {
        DisExcpt("SYF_EXCO_EXCO_SettlementAtMaturity.js", e);
    }
}
...
csFuncLevelProto.SYF_EXCO_CHK_MODULE_NAME = function() {
    try {

        if (SYS_ORG_FUNCTION_SHORT_NAME == "Establishment" || SYS_ORG_FUNCTION_SHORT_NAME == "FinanceRepay") {
            document.MAINFORM.TEMP_MODULE_NAME.value = 'CFNC';
        } else {
            document.MAINFORM.TEMP_MODULE_NAME.value = 'EXCO';
        }
    } catch (e) {
        DisExcpt("SYF_EXCO_EXCO_SettlementAtMaturity.js", e);
    }
...
csFuncLevelProto.FLD_EXCO_DRWNG_AMT_COLL_CCY_onchange = function(event) {
    try {
        ...
        //if (SYS_BeFloat(document.MAINFORM.DRWNG_AMT_COLL_CCY.value) > SYS_BeFloat(document.MAINFORM.COLL_TRX_CCY_BAL.value)) { on 20190226 65702;
        if (SYS_BeFloat(document.MAINFORM.DRWNG_AMT_COLL_CCY.value) > SYS_BeFloat(document.MAINFORM.TEMP_COLL_BAL_COL_CCY.value)) {
            ...
            document.MAINFORM.DRWNG_AMT_COLL_CCY.value = document.MAINFORM.TEMP_COLL_BAL_COL_CCY.value;
        }
        SYF_EXCO_CALL_AMT_RCVD_OUR_CHGS();
        SYF_EXCO_CALL_NEW_OTST_BAL();
