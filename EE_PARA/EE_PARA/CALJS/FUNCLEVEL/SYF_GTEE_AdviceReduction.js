...
csFuncLevelProto.SYF_GTEE_AMD_DETAILS = function() {
    try {
        ...
        DisExcpt("SYF_GTEE_AdviceReduction.js", e);
    }
}
...
csFuncLevelProto.SYF_GTEE_NEW_BASE_LCY = function() {
    try {
        ...
        var value = document.MAINFORM.NEW_BASE_BAL.value;
        if (SYM_GTEE_CHK_NEG(value)) {
            alert("The amount field do not accept negative values");
            document.MAINFORM.NEW_BASE_BAL.value = 0;
        }
    } catch (e) {
        DisExcpt("SYF_GTEE_AdviceReduction.js", e);
    }
...
csFuncLevelProto.FLD_GTEE_AMD_DT_onchange = function(event) {
    try {
        ...
        if (nDays > 0) {
            alert("Amend date should not accept before Registration date");
            document.MAINFORM.AMD_DT.value = '';
            return false;
        } else if (nDays1 > 0) {
            alert("Amend date should not accept after Expiry date");
            document.MAINFORM.AMD_DT.value = '';
            return false;
        } else {
            return true;
...
csFuncLevelProto.FLD_GTEE_CHARGES_onchange = function(event) {
    try {
        if (document.MAINFORM.CHARGES.value != '') {
            SYT_ChangeFldClass(document.MAINFORM.X798_32B_GUAR_CCY, 'M');
            SYT_ChangeFldClass(document.MAINFORM.X798_32B_AMT, 'M');
        } else {
            SYT_ChangeFldClass(document.MAINFORM.X798_32B_GUAR_CCY, 'O');
            SYT_ChangeFldClass(document.MAINFORM.X798_32B_AMT, 'O');
        }
    } catch (e) {
        DisExcpt("SYF_GTEE_AdviceReduction.js", e);
    }
...
csFuncLevelProto.FLD_GTEE_X798_23X_CODE_onchange = function(event) {
    try {
        if (document.MAINFORM.X798_23X_CODE.value != '') {
            document.MAINFORM.TEMP_X798_23X_CODE.value = '/' + document.MAINFORM.X798_23X_CODE.value + '/';
        }
    } catch (e) {
        DisExcpt("SYF_GTEE_AdviceReduction.js", e);
    }
...
csFuncLevelProto.FLD_GTEE_X798_32B_AMT_onchange = function(event) {
    try {
        var value = document.MAINFORM.X798_32B_AMT.value;
        document.MAINFORM.X798_32B_AMT.value = SYT_AmtFormat(document.MAINFORM.NEW_GTEE_AMT.value, value);
        if (SYM_GTEE_CHK_NEG(value)) {
            alert("Amount should not allow negative values");
            document.MAINFORM.X798_32B_AMT.value = "0.00";
        } else if (document.MAINFORM.X798_32B_AMT.value.toString().replaceAll(',', '').length > 18) {
            alert("Amount field should not allowed more than 18! character");
            document.MAINFORM.X798_32B_AMT.value = "0.00";
        }
    } catch (e) {
        DisExcpt("SYF_GTEE_AdviceReduction.js", e);
...
csFuncLevelProto.FLD_GTEE_X798_AMT_34X_onchange = function(event) {
    try {
        var value = document.MAINFORM.X798_AMT_34X.value;
        document.MAINFORM.X798_AMT_34X.value = SYT_AmtFormat(document.MAINFORM.NEW_GTEE_AMT.value, value);
        if (SYM_GTEE_CHK_NEG(value)) {
            alert("Amount field should not accept negative values");
            document.MAINFORM.X798_AMT_34X.value = "0.00";
        } else if (document.MAINFORM.X798_AMT_34X.value.toString().replaceAll(',', '').length > 18) {
            alert("Amount field should not allowed more than 18! character");
            document.MAINFORM.X798_AMT_34X.value = "0.00";
        }
    } catch (e) {
...
csFuncLevelProto.FLD_GTEE_X798_CRE_DATE_onchange = function(event) {
    try {
        if (SYS_GetSubDays(document.MAINFORM.ISSUE_DT.name, document.MAINFORM.X798_CRE_DATE.name) < 0) {
            SYS_CheckError(document.MAINFORM.X798_CRE_DATE, "Creation Date must be later than Issue Date!");
            document.MAINFORM.X798_CRE_DATE.value = '';
        } else if ((SYS_GetSubDays(document.MAINFORM.NEW_EXPIRY_DT.name, document.MAINFORM.X798_CRE_DATE.name) >= 0)) {
            SYS_CheckError(document.MAINFORM.X798_CRE_DATE, "Creation Date is not allowed later than Expiry/Review Date");
            document.MAINFORM.X798_CRE_DATE.value = '';
        }
    } catch (e) {
        DisExcpt("SYF_GTEE_AdviceReduction.js", e);
