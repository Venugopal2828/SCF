...
csFuncLevelProto.InitValues = function() {
    try {
        ...
        document.MAINFORM.FA_BUSI_TYPE.value = 'ABLF';
        document.MAINFORM.CNTR_STATUS.value = 'A';
        document.MAINFORM.LOAN_TIMES.value = 0;
        document.MAINFORM.FA_TTL_LOAN_BAL.value = 0;
        document.MAINFORM.TRX_DT.value = SYS_BUSI_DATE;
        document.MAINFORM.MSG_TYPE.value = 'ABL.001.AGR';
        SYS_GetRefNo('ABLF_CNTR', 'SYF_ABLF_SetMainRef');


    } catch (e) {
        DisExcpt("SYF_ABLF_SignAgreement.js", e);
    }
}
...
csFuncLevelProto.ConfirmBusinessCall = function() {
    try {
        ...
        document.MAINFORM.FA_LMT_BAL.value = SYS_BeFloat(document.MAINFORM.FA_LMT_AMT.value);
    } catch (e) {
        DisExcpt("SYF_ABLF_SignAgreement.js", e);
    }
}

csFuncLevelProto.SYF_ABLF_InqCustomerID = function() {
    try {

        SYS_InqCUBK_byCondition('FA_CUST_ID', '1');
    } catch (e) {
        DisExcpt("SYF_ABLF_SignAgreement.js", e);
    }
}

csFuncLevelProto.SYF_ABLF_GetCustomerInfo = function() {
    try {

        SYS_GetCUBK('FA_CUST_ID', 'FA_CUST_ID', 'SYF_ABLF_Check_CUST_ID()');
    } catch (e) {
        DisExcpt("SYF_ABLF_SignAgreement.js", e);
    }
}
...
csFuncLevelProto.SYF_ABLF_SetMainRef = function(ref) {
    try {

        var UnitCode = SYS_BUSI_UNIT;
        var dt = getDate(SYS_DATE_FORMAT, SYS_BUSI_DATE);
        year = dt.substr(2, 2);
        document.MAINFORM.C_MAIN_REF.value = 'ABLF' + UnitCode + year + ref;
        document.MAINFORM.FA_CNTR_REF.value = 'ABLF' + UnitCode + year + ref;
    } catch (e) {
        DisExcpt("SYF_ABLF_SignAgreement.js", e);
    }
}
...
csFuncLevelProto.SYF_ABLF_Check_CUST_ID = function() {
    try {

        document.MAINFORM.FA_TEMP3.value = '';
        SYS_GetTableDataByRule_S('Check_SignAgreement_CUST_ID', '1', 'Y');
        if (document.MAINFORM.FA_TEMP3.value != null && document.MAINFORM.FA_TEMP3.value != '' && document.MAINFORM.FA_TEMP3.value != 'null') {
            SYS_CheckError(document.MAINFORM.FA_CUST_ID, 'This customer has already signed an agreement!');
            return false;
        }
        return true;
    } catch (e) {
...
csFuncLevelProto.FLD_ABLF_FA_CUST_ID_onchange = function(event) {
    try {
        SYF_ABLF_GetCustomerInfo();
    } catch (e) {
        DisExcpt("SYF_ABLF_SignAgreement.js", e);
    }
}
