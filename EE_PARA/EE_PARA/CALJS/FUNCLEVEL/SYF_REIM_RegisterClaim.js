...
csFuncLevelProto.PreconditionOnInit = function() {
    try {
        ...
        DisExcpt("SYF_REIM_RegisterClaim.js", e);
    }
}
...
csFuncLevelProto.InitValues = function() {
    try {
        ...
        document.MAINFORM.CLM_BK_ID.value = '';
        document.MAINFORM.CLM_BK_NM.value = '';
        document.MAINFORM.CLM_DT.value = SYS_DATE;

        if (SYS_BeInt(document.MAINFORM.CLM_CNTR.value) == 0) {
            //document.MAINFORM.PEND_REIM_INST_BAL.value = document.MAINFORM.REIM_INST_BAL.value;
            //document.MAINFORM.PEND_REIM_CONF_BAL.value = document.MAINFORM.REIM_CONF_BAL.value;
            document.MAINFORM.PEND_REIM_INST_BAL.value = 0;
            document.MAINFORM.PEND_REIM_CONF_BAL.value = 0;
            // document.MAINFORM.NEW_REIM_INST_BAL.value = document.MAINFORM.REIM_INST_BAL.value;
            // document.MAINFORM.NEW_REIM_CONF_BAL.value = document.MAINFORM.REIM_CONF_BAL.value;
        } else {
        ...
        document.MAINFORM.CLM_CNTR.value = SYS_BeInt(document.MAINFORM.CLM_CNTR.value) + 1;
...
csFuncLevelProto.SYF_REIM_Cal_NEW_REIM_INST_BAL = function() {
    try {
        ...
        var NEW_REIM_INST_BAL = SYS_BeFloat(document.MAINFORM.NEW_REIM_INST_BAL.value);
...
csFuncLevelProto.SYF_REIM_Chk_CLM_TRX_CCY_AMT = function() {
    try {
        ...
        if (CLM_TRX_CCY_AMT > REIM_INST_BAL) {
            SYS_CheckError(document.MAINFORM.CLM_TRX_CCY_AMT, "Principal Amount Claimed <= Reimbursement Balance");
...
csFuncLevelProto.SYF_REIM_Cal_NET_CLAIM_ISSBK = function() {
    try {
        ...
        NET_CLAIM_ISSBK = CLM_TRX_CCY_AMT + ADD_AMT_CLMD + CHG_PLUS - CHG_MINUS;
        document.MAINFORM.NET_CLAIM_ISSBK.value = SYT_AmtFormat(document.MAINFORM.LC_CCY.value, NET_CLAIM_ISSBK);
    } catch (e) {
        DisExcpt("SYF_REIM_RegisterClaim.js", e);
    }
}
...
csFuncLevelProto.SYF_REIM_Cal_CLM_CNTR = function() {
    try {
        ...
        var NO = SYS_BeInt(document.MAINFORM.CLM_CNTR.value);
        var NR;
        if (NO < 10) {
            NR = '/0' + NO;
        } else {
...
csFuncLevelProto.SYF_REIM_Cal_CHG_MINUS = function() {
    try {
        ...
        if (CHG_PLUS != 0) {
            CHG_MINUS = 0;
            document.MAINFORM.CHG_MINUS.value = SYT_AmtFormat(document.MAINFORM.LC_CCY.value, CHG_MINUS);
            SYT_ChangeFldClass(document.MAINFORM.CHG_MINUS, 'O');
...
csFuncLevelProto.SYF_REIM_Cal_CHG_PLUS = function() {
    try {
        ...
        if (CHG_MINUS != 0) {
            CHG_PLUS = 0;
            document.MAINFORM.CHG_PLUS.value = SYT_AmtFormat(document.MAINFORM.LC_CCY.value, CHG_PLUS);
            SYT_ChangeFldClass(document.MAINFORM.CHG_PLUS, 'O');
        } else if (CHG_PLUS == 0 && CHG_MINUS == 0) {
            SYT_ChangeFldClass(document.MAINFORM.CHG_MINUS, 'O');
            SYT_ChangeFldClass(document.MAINFORM.CHG_PLUS, 'O');
