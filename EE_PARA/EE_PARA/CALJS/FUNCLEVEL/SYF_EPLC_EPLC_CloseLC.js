...
csFuncLevelProto.InitValues = function() {
    try {
        ...
        SYF_EPLC_CHK_OUT_DRAW();
        SYF_EPLC_CHK_LIABILITY();

        document.MAINFORM.TEMP_AMT18.value = document.MAINFORM.CONF_BAL.value;
        document.MAINFORM.TEMP_AMT19.value = document.MAINFORM.LIAB_BAL.value;
        document.MAINFORM.OLD_CONF_BAL.value = document.MAINFORM.CONF_BAL.value;
        document.MAINFORM.OLD_LC_BAL.value = document.MAINFORM.LC_BAL.value;
        document.MAINFORM.OLD_LIAB_BAL.value = document.MAINFORM.LIAB_BAL.value;
    } catch (e) {
        DisExcpt("SYF_EPLC_EPLC_CloseLC.js", e);
    }
}
...
csFuncLevelProto.ConfirmBusinessCall = function() {
    try {
        ...
        document.MAINFORM.LC_BAL.value = SYT_AmtFormat(document.MAINFORM.LC_CCY.value, 0);
...
csFuncLevelProto.SYF_EPLC_CHK_ADD_CHARGE = function() {
    try {
        ...
        if (document.MAINFORM.WRITE_OFF_CHG_FLG.value == 'NO') {
            SYS_CheckError(document.MAINFORM.WRITE_OFF_CHG_FLG, "There are outstanding charges. Please use the Settle Charges func to clear them.");
            ...
            return false;
        } else if (document.MAINFORM.WRITE_OFF_CHG_FLG.value == 'YES') {
            document.MAINFORM.CHG_FLD_ALL_CHARGE_AT.value = '3';
            defChgArr = Chg.Screen.getAllDefCharge();
            for (i = 0; i < defChgArr.length; i++) {
                charge = defChgArr[i];
                charge.setChargeAt("3");
                charge.chargeAtOnchange();
            }
        } else {
...
csFuncLevelProto.SYF_EPLC_CHK_LIABILITY = function() {
    try {

        //added by zoe 20081128
        /*
if(SYS_BeFloat(document.MAINFORM.CONF_BAL.value)>0){
alert("You cannot close this file with outstanding liability");
return false;
}else{
return true;
}
...
csFuncLevelProto.SYF_EPLC_CHK_OUT_DRAW = function() {
    try {

        var arr_result; // Utility Auto Fix Comments
        var i; // Utility Auto Fix Comments
        var nTTL_CLAIM_BAL; // Utility Auto Fix Comments
        var sql; // Utility Auto Fix Comments
        //for check outstanding drawings
        if (document.MAINFORM.NO_OF_DRAW.value != "") {
            SYS_MULTI_DATA = "";
            //	sql=" WHERE C_MAIN_REF='" + document.MAINFORM.C_MAIN_REF.value + "'";
            //SYS_GetTableMultiDataToArray_S("EPLC_MASTER",sql,"PENDING_PRES_BAL",true);
            SYS_GetTableDataByRule_S("SYF_EPLC_CHK_OUT_DRAW_0", "1", true, true);
            arr_result = SYS_MULTI_DATA.toString().split(",");
            nTTL_CLAIM_BAL = 0;
            for (i = 1; i < arr_result.length; i++) { // Utility Auto Fix Comments
                nTTL_CLAIM_BAL += SYS_BeFloat(arr_result[i]);
            }
            if (nTTL_CLAIM_BAL > 0) {
                alert("There are outstanding drawings awaiting settlement.");
                return false;
            } else {
                return true;
            }
...
csFuncLevelProto.ConfirmBusinessCheck = function() {
    try {
        ...
        //return (SYF_EPLC_CHK_OUT_DRAW()&&SYF_EPLC_CHK_LIABILITY());
        return (SYF_EPLC_CHK_OUT_DRAW());
        /*
if(!SYM_EPLC_CHK_FOREIGN_CHG()){
return false;
}
*/
    } catch (e) {
...
csFuncLevelProto.FLD_EPLC_WRITE_OFF_CHG_FLG_onchange = function(event) {
    try {
        SYF_EPLC_CHK_ADD_CHARGE();
    } catch (e) {
        DisExcpt("SYF_EPLC_EPLC_CloseLC.js", e);
