...
csFuncLevelProto.SYF_FAEF_Cal_ExcelUploaded = function() {
    try {
        ...
        var doObj_VerifyInv; // Utility Auto Fix Comments
        ...
        doObj_VerifyInv = SYS_getDoByXpath('VerifyInv');
        SYF_FAEF_Cal_forChildtoMainScreen(doObj_VerifyInv);
    } catch (e) {
...
csFuncLevelProto.SYF_FAEF_Cal_forChildtoMainScreen = function(node, recordId, status) {
    try {
        ...
        nosum = SYS_getcurrRecordCount("VerifyInv");
...
csFuncLevelProto.SYF_FAEF_Cal_FA_INV_CCY = function() {
    try {
        ...
        SYS_refreshChildDoValue("VerifyInv", 'FA_DOC_CCY', ccy);
    } catch (e) {
...
csFuncLevelProto.FLD_FAEF_FA_EF_HAN_CHG_PAMT_onchange = function(event) {
    try {
        ...
        var record; // Utility Auto Fix Comments
        var type; // Utility Auto Fix Comments
        //SYS_refreshChildDoValue('VerifyInv','FA_EF_HAN_CHG_PAMT',document.MAINFORM.FA_EF_HAN_CHG_PAMT.value);
        node = SYS_getDoByXpath('VerifyInv');
        mData = [];
        arrayvalue = SYS_getRecords(node);
        for (i = 0, len = arrayvalue.length; i < len; i++) {
...
csFuncLevelProto.FLD_FAEF_FA_IF_HAN_CHG_PAMT_onchange = function(event) {
    try {
        ...
        var record; // Utility Auto Fix Comments
        var type; // Utility Auto Fix Comments
        //SYS_refreshChildDoValue('VerifyInv','FA_IF_HAN_CHG_PAMT',document.MAINFORM.FA_IF_HAN_CHG_PAMT.value);
        if (document.MAINFORM.FA_BUSI_TYPE.value == 'EF') {
            node = SYS_getDoByXpath('VerifyInv');
            mData = [];
            arrayvalue = SYS_getRecords(node);
