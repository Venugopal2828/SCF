...
csFuncLevelProto.SYF_IPLC_Cal_NO_OF_DRAW = function() {
    try {
        ...
        DisExcpt("SYF_IPLC_IPLC_DocumentRegister_CE.js", e);
    }
}
...
csFuncLevelProto.InitValues = function() {
    try {
        ...
        //document.MAINFORM.DOC_STAT.value='To be Checked';
        SYT_Cal_LOCAL_AMT('LC_CCY', 'LC_BAL', 'LOCAL_CCY', 'LOCAL_AMT', 'LOCAL_RATE');

        //sFieldList = "C_EVENT_STATUS";
        SYS_GetTableDataByRule_S('SYF_IPLC_IPLC_DocumentRegister_InitValues_0', '1', 'Y', "Y");
        if (SYS_MULTI_DATA.length > 0) {
            SYS_highTrxButton("_cancel");
            alert("Another transaction is belong to this LC in processing! \n Please Release First !");
        }
    } catch (e) {
        DisExcpt("SYF_IPLC_IPLC_DocumentRegister_CE.js", e);
    }
...
csFuncLevelProto.PostconditionOnInit = function() {
    try {
        ...
        document.MAINFORM.ORIGINAL_LC_BAL.value = document.MAINFORM.LC_BAL.value; //Jax added 2020/5/15
        ...
        if (SYS_FUNCTION_TYPE != 'RE' && SYS_FUNCTION_TYPE != 'IQ' && SYS_FUNCTION_TYPE != 'EC') {
            ...
            document.MAINFORM.PRES_AMT.value = '';
...
csFuncLevelProto.FLD_IPLC_PRES_AMT_onchange = function(event) {
    try {
        ...
        if (SYS_BeFloat(document.MAINFORM.LC_BAL.value) < SYS_BeFloat(document.MAINFORM.PRES_AMT.value)) {
            alert("The Presentation amount should not exceed the LC balance");
            document.MAINFORM.PRES_AMT.value = 0;
        }
