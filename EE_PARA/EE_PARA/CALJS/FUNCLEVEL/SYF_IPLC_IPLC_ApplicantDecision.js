...
csFuncLevelProto.InitValues = function() {
    try {
        ...
        DisExcpt("SYF_IPLC_IPLC_ApplicantDecision.js", e);
    }
}
...
csFuncLevelProto.ConfirmBusinessCall = function() {
    try {
        ...
        document.MAINFORM.FURTHER_IDENTITY.value=document.MAINFORM.X752_FUR_IDEN_23.value; 
    } catch (e) {
        DisExcpt("SYF_IPLC_IPLC_ApplicantDecision.js", e);
    }
}
...
csFuncLevelProto.SYF_IPLC_MT798_FLAG = function() {
    try {
        ...
        var RES = document.MAINFORM.APPL_RESP_DISC.value;
        if (document.MAINFORM.APPLY_FLG.value == 'YES') {
            EEHtml.getElementById('Z').style.display = '';
            SYT_EnableDivClass('Z_div');
            // 20200311 Added S;
            if (RES == 'Discrepancies Accepted' || RES == 'Discrepancies Accepted Provided') {
                document.MAINFORM.SUB_MESS_TYPE.value = '731';
                document.MAINFORM.ADV_PRES_BY.value = 'No';
                SYT_ChangeFldClass(document.MAINFORM.DOC_DISP_FLG,'O');
                SYT_ChangeFldClass(document.MAINFORM.DISC_DET,'O');
            } else if(RES == ''){
                ...
                document.MAINFORM.SUB_MESS_TYPE.value = '733';
                document.MAINFORM.ADV_PRES_BY.value = 'MT734';
                SYT_ChangeFldClass(document.MAINFORM.DOC_DISP_FLG,'M');
                SYT_ChangeFldClass(document.MAINFORM.DISC_DET,'M');            	
            }
            // 20200311 Added E;
        ...
        if (RES == 'Discrepancies Accepted' || RES == 'Discrepancies Accepted Provided'||RES == '') {
                document.MAINFORM.ADV_PRES_BY.value = 'No';
                SYT_ChangeFldClass(document.MAINFORM.DOC_DISP_FLG,'O');
                SYT_ChangeFldClass(document.MAINFORM.DISC_DET,'O');
            }else{
...
csFuncLevelProto.FLD_IPLC_APPL_RESP_DISC_onchange = function(event) {
    try {
        SYF_IPLC_MT798_FLAG();
        /*if (document.MAINFORM.APPL_RESP_DISC.value == 'Discrepancies Accepted' || document.MAINFORM.APPL_RESP_DISC.value == 'Discrepancies Accepted Provided'||document.MAINFORM.APPL_RESP_DISC.value == '') {
