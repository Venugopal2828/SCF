...
csFuncLevelProto.SYF_GTEE_AMD_DETAILS = function() {
    try {
        ...
        DisExcpt("SYF_GTEE_AmendGuaranteeFrCoE.js", e);
    }
}
...
csFuncLevelProto.SYF_GTEE_Change_52_tag = function() {
    try {
        ...
        if (document.MAINFORM.ISSUE_BK_SW_ADD.value == 'A') {
...
csFuncLevelProto.InitValues = function() {
    try {
        ...
        document.MAINFORM.GTEE_BAL_LOCAL.value = document.MAINFORM.GTEE_BAL.value;
...
csFuncLevelProto.SYF_GTEE_REFORMAT_13E = function() {
    try {
        ...
        if (date_send != '') {
            ...
            document.MAINFORM.X798_CRE_DATE.value = day_send + '-' + month_send + '-' + year_send;
        }
    } catch (e) {
        DisExcpt("SYF_GTEE_AmendGuaranteeFrCoE.js", e);
    }
...
csFuncLevelProto.FLD_GTEE_CONTR_GTEE_EXP_onchange = function(event) {
    try {
        var nDays = SYS_GetSubDays(document.MAINFORM.CONTR_GTEE_EXP.name, document.MAINFORM.AMD_DT.name);
        var nDays1 = SYS_GetSubDays(document.MAINFORM.EXPIRY_DT.name, document.MAINFORM.CONTR_GTEE_EXP.name);
        if (nDays > 0) {
            alert("Counter Guarantee Expiry date should not accept before Amendment date");
            document.MAINFORM.CONTR_GTEE_EXP.value = '';
            return false;
        } else if (nDays1 > 0) {
            alert("Counter Guarantee Expiry date should not accept after Expiry date");
            document.MAINFORM.CONTR_GTEE_EXP.value = '';
            return false;
        } else {
            return true;
...
csFuncLevelProto.FLD_GTEE_NEW_EXPIRY_DT_onchange = function(event) {
    try {
        ...
        if (nDays > 0) {
            alert("New expiry date should not accept before Amendment date");
