...
csFuncLevelProto.SYF_IPLC_Change_APLB_RULE = function() {
    try {
        ...
        DisExcpt("SYF_IPLC_IPLC_IssueTransferLC.js", e);
    }
}
...
csFuncLevelProto.InitValues = function() {
    try {
        ...
        document.MAINFORM.ISSUE_DT.value = document.MAINFORM.MAST_START_DT.value;
...
csFuncLevelProto.SYF_IPLC_CHK_LATEST_SHIP_DT = function() {
    try {
        ...
        if (nDays1 < 0) {
            alert('LC Issue date should not be earlier than Master LC Issue date!');
        }
        if (nDays2 < 0) {
        ...
        if (nDays5 < 0) {
            alert('LC Expiry Date should not be later than Master LC Expiry Date!');
        }
        if (nDays6 < 0) {
            SYS_CheckError(document.MAINFORM.ISSUE_DT, 'The LC Issue Date should be later than Master LC Expiry Date');
