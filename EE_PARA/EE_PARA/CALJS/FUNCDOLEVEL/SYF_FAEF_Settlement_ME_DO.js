function MultiCreditSummary_MultiCredit_OnDeSelected(node, record, recordId) {
    try {
        ...
        DisExcpt("SYF_FAEF_Settlement_ME_DO.js", e);
    }
}
...
function SYF_FAEF_getDOdata_Settle_New(node, recordId, status) {
    try {
        ...
        if (search_by == 'Loan') {
            ...
            /* Modify for Enhance 3 on 20220811; S*/
            if(search_by == 'Unfinanced Invoices'){
                var aa = SYS_getDoByXpath('Settle_New');
                aa.clearAllDataSets(true);
                var objGetdateRule = {
           ...
           if(search_by == 'All') {
              ...
              /* Modify for Enhance 3 on 20220811; E*/
        }
...
//            if (parentloantimes == 1 && parentpaidflg == '1') {   AutoFinancing not update INVC_MASTER level
                 if (parentpaidflg == '1') {
                   ...
                   for (var i = 0, len = loanRecords.length; i < len; i++) {
                    ...
                    var rebateamt = SYS_getValFromRec(loanRecord, 'FA_REBATE_AMT');
                    var exten_prl = SYS_getValFromRec(loanRecord, 'EXTEN_PRCPL');  //20241212 for rollover;
                    if(rebateamt==''||rebateamt ==null){
                    rebateamt=0;
                    }
                    ...
                    settleRecord = SYS_setValToRec(settleRecord, 'EXTEN_PRCPL', exten_prl);
                                   if(exten_prl != 0){
                   
                         settleRecord = SYS_setValToRec(settleRecord, 'FA_TEMP_AMT7', exten_prl);
                      }
                      else{
                     	settleRecord = SYS_setValToRec(settleRecord, 'FA_TEMP_AMT7', tempEbal);
                  	 }
                   //for requirement1.3
                   var intChgType = SYS_getValFromRec(loanRecord, 'FA_INT_CHG_TYPE');
                   var pmtValDt =  SYS_getValFromRec(loanRecord, 'TRX_DT');
                   var grace = SYS_getValFromRec(loanRecord, 'FA_TEMP_GRAC');
                   var FA_INVOICE_END_DT;
                   SYS_CalEndWorkingDate_S(SYS_BANK_COUNTRY, loanDueDt, grace, document.MAINFORM.FA_INVOICE_END_DT.name, 'A', 'N', 'N');
                   var subdays1 = SYS_GetSubDays(document.MAINFORM.FA_INVOICE_END_DT.name, document.MAINFORM.TRX_DT.name);
				   loanValDt = SYS_getValFromRec(loanRecord, 'FA_LOAN_VAL_DT');
				   document.MAINFORM.FA_LOAN_VAL_DT_NEW_LOAN.value = loanValDt;
				   var subdays2 = SYS_GetSubDays(document.MAINFORM.FA_LOAN_VAL_DT_NEW_LOAN.name, document.MAINFORM.TRX_DT.name);
                   if(subdays1 < 0 ||subdays1 == 0){
                       loanRecord = SYS_setValToRec(loanRecord, 'FA_TEMP_RT_TYPE1', 'No'); // update overdue flag;
                   }
                   else {
                   	loanRecord = SYS_setValToRec(loanRecord, 'FA_TEMP_RT_TYPE1', 'Yes'); // update overdue flag;  RDCSBANK241210913SBR
                   	}
                   ...
                   {  	
                       ...
                       rebateamt= SYS_BeFloat(invint) * SYS_BeFloat(document.MAINFORM.FA_REBATE_RATE.value) / 100;
                       var netInt = SYS_FloatSub(SYS_BeFloat(invint),SYS_BeFloat(invint) * SYS_BeFloat(document.MAINFORM.FA_REBATE_RATE.value) / 100);
                       loanRecord = SYS_setValToRec(loanRecord, 'FA_REBATE_AMT', rebateamt);
                       loanRecord = SYS_setValToRec(loanRecord, 'FA_LOAN_INT_SM_AMT', netInt);
                      
                   }
                       settleRecord = SYS_setValToRec(settleRecord, 'FA_REBATE_AMT', rebateamt);
                   ...
                   var paid_loan = SYS_getValFromRec(settleRecord, 'FA_TEMP_AMT7');
                    loanRecord = SYS_setValToRec(loanRecord, 'FA_TEMP_AMT9', SYT_AmtFormat(document.MAINFORM.FA_DOC_CCY.value, paid_loan));
                   //for requirement1.5
...
function Settle_New_Settle_loan(node, recordId, status) {
    try {
        ...
        for (i = 0, len = arrayvalue.length; i < len; i++) {
            ...
            fldValue = SYS_getValFromRec(record, 'EXTEN_PRCPL');
            if (fldValue == '' || fldValue == null) {
                record = SYS_setValToRec(record, 'EXTEN_PRCPL', 0);
            }
            mData.push(record);
        }
        ...
        if (loantimes == 1 && paidflg == '1') {
            for (i = 0, len = arrayvalue.length; i < len; i++) {
                ...
                var EXTEN = document.MAINFORM.EXTEN_PRCPL.value; //rollover;
                if(EXTEN != 0){
                 document.MAINFORM.FA_TEMP_AMT7.value = EXTEN;
                 }
                else{
               document.MAINFORM.FA_TEMP_AMT7.value =loanbal;
                }
               ...
               // subdays1 = SYS_GetSubDays(document.MAINFORM.FA_LOAN_DUE_DT.name, document.MAINFORM.TRX_DT.name);
                  var grace = SYS_getValFromRec(record, 'FA_TEMP_GRAC');
                   var FA_INVOICE_END_DT;
                   SYS_CalEndWorkingDate_S(SYS_BANK_COUNTRY, loanDueDt, grace, document.MAINFORM.FA_LOAN_DUE_DT.name, 'A', 'N', 'N');
                   var subdays1 = SYS_GetSubDays(document.MAINFORM.FA_LOAN_DUE_DT.name, document.MAINFORM.TRX_DT.name);
				   
				    //niamh:eloan auto amz->acc new loan FA_LOAN_VAL_DT < TRX_DT
				   loanValDt = SYS_getValFromRec(record, 'FA_LOAN_VAL_DT');
				   document.MAINFORM.FA_LOAN_VAL_DT_NEW_LOAN.value = loanValDt;
				   var subdays2 = SYS_GetSubDays(FA_LOAN_VAL_DT_NEW_LOAN.name, document.MAINFORM.TRX_DT.name);
				   
                   if(subdays1 < 0 ){
                       record = SYS_setValToRec(record, 'FA_TEMP_RT_TYPE1', 'No'); // update overdue flag;
                   }
                   else {
                   	record = SYS_setValToRec(record, 'FA_TEMP_RT_TYPE1', 'Yes'); // update overdue flag;  
                   	}        
                ...
                if (subdays1 < 0 && intChgType == '1') {
                    ...
                    //niamh:eloan auto amz->acc new loan FA_LOAN_VAL_DT > TRX_DT bug
					if(subdays2 > 0){
						SYS_InqGapi_S('FAEF_Settle_Inquire_ACC');
					}
                ...
                //Edit by Sunny
                record = SYS_setValToRec(record, 'FA_INV_LOAN_EBAL', 0);
                var loan = SYT_AmtFormat(document.MAINFORM.FA_DOC_CCY.value, document.MAINFORM.FA_TEMP_AMT7.value);
                 record = SYS_setValToRec(record, 'FA_TEMP_AMT9', loan);
                mData.push(record);
                SYS_reLoadGrid(node, mData);
                //for requirement1.5
            ...
            for (i = 0, len = arrayvalue.length; i < len; i++) {
              ...
              //  subdays1 = SYS_GetSubDays(document.MAINFORM.FA_LOAN_DUE_DT.name, document.MAINFORM.TRX_DT.name);
                ...
                if (subdays1 < 0 && intChgType == '1') {
					...
					var pmtamt = SYT_AmtFormat(document.MAINFORM.FA_DOC_CCY.value, document.MAINFORM.FA_PMT_AMT.value);
					var ppamt = SYS_FloatMul(pmtamt, percent) / 100;
					var pamt = SYS_FloatAdd(ppamt,invint);
					var TempLoanBal=SYS_getValFromRec(record, 'FA_TEMP_LOAN_EBAL');
					record = SYS_setValToRec(record, 'FA_LOAN_PAID_AMT', pamt);
					record = SYS_setValToRec(record, 'FA_LOAN_PPAID_AMT', ppamt);

					//FA_INV_LOAN_BAL
					record = SYS_setValToRec(record, 'FA_INV_LOAN_EBAL', SYS_FloatSub(TempLoanBal - ppamt));
					
					
                }
				mData.push(record);
			   var  EXTEN1= SYS_getValFromRec(record, 'EXTEN_PRCPL');
			   SYS_reLoadGrid(node, mData);
			   PRINAMT = SYS_getFieldSumValue(node, "FA_LOAN_PPAID_AMT", 2);
                if (loanbal > ppaidamt) {
                  //document.MAINFORM.FA_TEMP_AMT7.value = document.MAINFORM.FA_PMT_AMT.value;//Niamh 2025-05-29
				  document.MAINFORM.FA_TEMP_AMT7.value = PRINAMT;
                ...
                var aaaa = SYS_FloatSub(document.MAINFORM.EXTEN_PRCPL.value,document.MAINFORM.FA_TEMP_AMT7.value);
                record = SYS_setValToRec(record, 'EXTEN_PRCPL', aaaa);
                var paid_eloan = SYT_AmtFormat(document.MAINFORM.FA_DOC_CCY.value, document.MAINFORM.FA_TEMP_AMT7.value);
                record = SYS_setValToRec(record, 'FA_TEMP_AMT9', paid_eloan);	
                mData.push(record);
            }
        }
        //for requirement2.5
