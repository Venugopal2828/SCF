function InvFinance(node, recordId, status) {
    try {
            ...
            if ('D' != status) {
                for (i = 0, len = arrayvalue.length; i < len; i++) {
                    ...
                    loanamt = SYS_BeFloat(lmtApprv) * (SYS_BeFloat(FA_LOAN_PERC) / 100);*/
                        ...
                        docRef = SYS_getValFromRec(record, 'FA_DOC_REF');
                        len2 = docRef.length - 13;
                        docId = docRef.substr(len2, 13);
                        invloan = docId + loanref;
        ...
        DisExcpt("SYF_FAEF_Financing_DO.js", e);
    }
}
            ...
            if ('D' != status) {
                for (i = 0, len = arrayvalue.length; i < len; i++) {
                    ...
                    if (reqLoanBal <= 0) {
                        ...
                        POLoanAvl = SYS_BeFloat(SYS_getValFromRec(record, 'PO_LOAN_AVL'));
...
function SYF_FAEF_getDOdata_POFinance() {
    try {
