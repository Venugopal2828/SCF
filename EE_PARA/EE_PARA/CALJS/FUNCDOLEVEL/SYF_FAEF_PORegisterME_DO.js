...
function SYF_FAEF_getDOdata_ChgDoDef() {
    try {
        ...
        DisExcpt("SYF_FAEF_PORegisterME_DO.js", e);
    }
}
...
function PORegister(node, recordId, status) {
    try {
		...
		pofinamt=SYS_BeFloat(SYS_getValueFromMain('FA_TTL_PO_AMT')) * SYS_BeFloat(SYS_getValueFromMain('PO_MAX_LOAN_PERC')) / 100;
		SYS_setValueToMain('PO_AVAL_FOR_FUNDING', pofinamt);
		document.MAINFORM.PO_AVAL_FOR_FUNDING.value = SYT_CCY_AMT(poCCY, document.MAINFORM.PO_AVAL_FOR_FUNDING.value);
