...
function EFRemitInv_OnDeSelected(scope, record, rowId) {
    try {
        ...
        DisExcpt("SYF_FADA_IFReciveCommission_DO.js", e);
    }
}
...
function SYF_FADA_getDOdata_EFRemitInv(node, recordId, status) {
    try {
        ...
        document.MAINFORM.FA_REMI_TTL_AMT.value = 0;
        SYS_GetDataForDO_S('IFRemitInv');
    } catch (e) {
        DisExcpt("SYF_FADA_IFReciveCommission_DO.js", e);
    }
}
...
function ExpRemi_FA_SEL_SUBTTL_DEDUCT_onchange() {
    try {
        var _do; // Utility Auto Fix Comments
        _do = SYS_getDoByXpath("ExpRemi"); // Utility Auto Fix Comments
        SYF_null_Invalid Function: F14050900492(_do); // Utility Auto Fix Comments
    } catch (e) {
        DisExcpt("SYF_FADAIFReciveCommission_DO.js", e);
    }
}