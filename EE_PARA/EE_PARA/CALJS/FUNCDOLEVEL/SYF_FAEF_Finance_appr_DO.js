function InvFinance(node, recordId, status) {
    try {
        ...
        var POLoanbal = SYS_BeFloat(document.MAINFORM.PO_LOAN_BAL.value);
        ...
        if (num > 0) {
            ...
            if ('D' != status) {
              ...
              document.MAINFORM.FA_REBATE_AMT.value = SYT_CCY_AMT(document.MAINFORM.FA_DOC_CCY.value, SYS_BeFloat(document.MAINFORM.FA_REBATE_AMT.value));//zoe for amtformat
              if(busitype =='POF' && POLoanbal > 0){
        ...
        DisExcpt("SYF_FAEF_Finance_appr_DO.js", e);
    }
}
