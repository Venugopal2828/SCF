...
function For_Fullpayment_GetDoData() {
    ...
    for (var i = 0, len = arrayvalue.length; i < len; i++) {
        ...
        record = SYS_setValToRec(record, 'FA_TEMP4', SYS_FUNCTION_SHORT_NAME);
        ...
        if (loantimes == 1 && paidflg == '1') {
               ...
               // subdays1 = SYS_GetSubDays(document.MAINFORM.FA_LOAN_DUE_DT.name, document.MAINFORM.TRX_DT.name);
                  var grace = SYS_getValFromRec(record, 'FA_TEMP_GRAC');
                   ...
                   if(subdays1 < 0 ){
                   	...
                   	record = SYS_setValToRec(record, 'FA_TEMP_RT_TYPE1', 'Yes'); // update overdue flag;  RDCSBANK241210913SBR
                   	}        
            ...
            for (i = 0, len = arrayvalue.length; i < len; i++) {
               ...
               var  EXTEN1= SYS_getValFromRec(record, 'EXTEN_PRCPL');
                if (loanbal > ppaidamt) {
                  document.MAINFORM.FA_TEMP_AMT7.value = document.MAINFORM.FA_PMT_AMT.value;
