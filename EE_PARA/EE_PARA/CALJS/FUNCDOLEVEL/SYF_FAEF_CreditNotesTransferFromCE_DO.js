...
function SYF_FAEF_getDOdata_ChgDoDef() {
    try {
        ...
        DisExcpt("SYF_FAEF_CreditNotesTransferFromCE_DO.js", e);
    }
}
...
function CreditNote1(node, recordId, status) {
    try {
        var num = SYS_getcurrRecordCount("CreditNote1");
        SYS_setValueToMain('FA_TTL_CRN_NO', num);
...
function CreditNote1_OnDeSelected(node, record, recordId) {
    try {
...
function CreditNote1_OnSelected(node, record, recordId) {
    try {
...
function SYF_FAEF_getDOdata_CreditNote1() {
    try {
