function R2DatasetRequired_CertificateDataSetRequired(node, recordId, status) {
    try {
        ...
        DisExcpt("SYF_TSUM_R2InitialBaselineSubmission_DO.js", e);
    }
}
...
function R2Goods_Adjustmtents(node, recordId, status) {
    try {
        if (status == 'A' || status == 'E') {
            ...
            SYS_setFieldValue(pNode, recorId, "TSU_TTL_NET_OUAMT", nTSU_TTL_NET_AMT);
            ...
            //@author Mia Huang @Date 20091021 Start-- Set TSU_TTL_AMT_sum to TSU_LINE_TTL_OUAMT
            SYS_setFieldValue(pNode, recorId, "TSU_LINE_TTL_OUAMT", TSU_TTL_AMT_sum);
            //@author Mia Huang @Date 20091021 End-- Set TSU_TTL_AMT_sum to TSU_LINE_TTL_OUAMT
            SYS_setFieldValue(pNode, recorId, "TSU_TTL_NET_AMT", TSU_TTL_NET_AMT);
            //@author Mia Huang @Date 20091021 Start-- Set TSU_TTL_NET_AMT to TSU_TTL_NET_OUAMT                 
            SYS_setFieldValue(pNode, recorId, "TSU_TTL_NET_OUAMT", TSU_TTL_NET_AMT);
            //@author Mia Huang @Date 20091021 Start-- Set TSU_TTL_NET_AMT to TSU_TTL_NET_OUAMT      
        }
    } catch (e) {
        DisExcpt("SYF_TSUM_R2InitialBaselineSubmission_DO.js", e);
...
function R2Goods_R2LineItemDetails7_Adjustmtents(node, recordId, status) {
    try {
        if (status == 'D') {
            ...
            var nTSU_TTL_AMT = SYS_BeFloat(nAMT) + SYS_BeFloat(nAdjust_sum) + SYS_BeFloat(sum_Freight) + SYS_BeFloat(sum_Tax);
            nTSU_TTL_AMT = SYS_BeFloat(nTSU_TTL_AMT);
            //Set nTSU_TTL_AMT to the field TSU_TTL_AMT and TSU_TTL_OUAMT
            SYS_setCurrNodeParentValue('R2LineItemDetails7', 'TSU_TTL_AMT', nTSU_TTL_AMT);
            SYS_setCurrNodeParentValue('R2LineItemDetails7', 'TSU_TTL_OUAMT', nTSU_TTL_AMT);
            ...
            SYM_TSUM_RecalAdjustTaxFreiW(node);
            // get uncle's sum
            ...
            SYS_setCurrNodeParentValue('R2Goods', "TSU_TTL_NET_OUAMT", nUTSU_TTL_AMT_sum);
...
function R2Goods_R2LineItemDetails7_FrghtChrgs(node, recordId, status) {
    try {
        if (status == 'D') {
            ...
            SYS_setCurrNodeParentValue('R2LineItemDetails7', 'TSU_TTL_OUAMT', TSU_TTL_AMT_sum);
            //Get Sum of TSU_TTL_AMT
            var TSU_TTL_AMT_sum = SYS_getFieldSumByXpath('TSU_TTL_AMT', 'R2Goods.R2LineItemDetails7');
            TSU_TTL_AMT_sum = SYS_BeFloat(TSU_TTL_AMT_sum);
            SYS_setCurrNodeParentValue('R2Goods', "TSU_LINE_TTL_AMT", TSU_TTL_AMT_sum);
		...
		var len=records.length;
		if(len==0)
		   SYT_ChangeFldClass(document.MAINFORM.TSU_FRGHT_TP,'O','N');
		   alert(0);
		else
			 SYT_ChangeFldClass(document.MAINFORM.TSU_FRGHT_TP,'M','N');
			 alert(len);
		//-e*/
            ...
            pRecord = SYS_getParentRecord(node);
            recorId = pRecord.recordID;
            SYS_setFieldValue(pNode, recorId, "TSU_TTL_AMT", nTSU_TTL_AMT);
            SYS_setFieldValue(pNode, recorId, "TSU_TTL_OUAMT", nTSU_TTL_AMT);
...
function R2Goods_R2LineItemDetails7_Tax3(node, recordId, status) {
    try {
        if (status == 'D') {
            ...
            var nTSU_TTL_AMT = SYS_BeFloat(nAMT) + SYS_BeFloat(nTax_sum) + SYS_BeFloat(sum_Freight) + SYS_BeFloat(sum_Adjust);
            nTSU_TTL_AMT = SYS_BeFloat(nTSU_TTL_AMT);

            //Set nTSU_TTL_AMT to the field TSU_TTL_AMT
            SYS_setCurrNodeParentValue('R2LineItemDetails7', 'TSU_TTL_AMT', nTSU_TTL_AMT);
            SYS_setCurrNodeParentValue('R2LineItemDetails7', 'TSU_TTL_OUAMT', nTSU_TTL_AMT);
            //Get Sum of TSU_TTL_AMT
            var TSU_TTL_AMT_sum = SYS_getFieldSumByXpath('TSU_TTL_AMT', 'R2Goods.R2LineItemDetails7');
...
function R2InvolvedPartyInfo_BuyrSdSubmitgBk(node, recordId, status) {
    try {
        var TSU_BYRBK_ID = SYS_getCurrDoScreenValue('TSU_BUYER_BK_ID');
        var TSU_BYRSUBMITB = SYS_getCurrDoScreenValue('TSU_BUYER_SUB_BK');
        var TSU_BYRBK_NAME = SYS_getCurrDoScreenValue('TSU_BUYER_BK_NM');
        var TSU_PARTY_CNTY = SYS_getCurrDoScreenValue('TSU_PARTY_CNTY');
        var record = [DoFrame.tree.getDONameFunction(node, '_') + recordId, "Buyer Side Submitting Bank", TSU_BYRBK_ID, TSU_BYRSUBMITB, TSU_BYRBK_NAME, TSU_PARTY_CNTY, 'A'];
        SYS_DistillInfo(node, recordId, status, record);
    } catch (e) {
        DisExcpt("SYF_TSUM_R2InitialBaselineSubmission_DO.js", e);
    }
...
function R2InvolvedPartyInfo_SellrSdSubmitgBk(node, recordId, status) {
    try {
        var TSU_SELLRBK_ID = SYS_getCurrDoScreenValue('TSU_SEL_BK_ID');
        var TSU_SELLRSUBMITBK = SYS_getCurrDoScreenValue('TSU_SEL_SUB_BK');
        var TSU_SELLRBK_NAME = SYS_getCurrDoScreenValue('TSU_SEL_BK_NM');
        var xpath = DoFrame.tree.getDONameFunction(node, '_');
        var record = [xpath + recordId, "Seller Side Submitting Bank", TSU_SELLRBK_ID, TSU_SELLRSUBMITBK, TSU_SELLRBK_NAME, '', 'A'];
        SYS_DistillInfo(node, recordId, status, record);
    } catch (e) {
        DisExcpt("SYF_TSUM_R2InitialBaselineSubmission_DO.js", e);
    }
