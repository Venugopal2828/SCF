function merge() {
	stp.log('js merge mx pacs.010col');
	...
	stp.setData("/AppHdr/Fr/FIId/FinInstnId/BICFI", "ROYCCAT2XXX");
	stp.setData("/AppHdr/To/FIId/FinInstnId/BICFI", j1);
	stp.setData("/AppHdr/BizMsgIdr", stp.xml(source, "domData/C_MAIN_REF"));

    stp.setData("/AppHdr/BizSvc", "swift.cbprplus.col.02");
    bizsvc='swift.cbprplus.col.02';
    stp.log('js bizsvc mx pacs.010   swift.cbprplus.col.02');
}
function demerge() {
	stp.setData("F72_NODOM", " from esp script. ");
	stp.log('js: demerge mx pacs.010col');
