...
var MODULE = DV.getParamFieldValue("MODULE");
DV.writeLog("*****CNPT_ID = " + CNPT_ID);
DV.writeLog("*****CNPT_SSI_CCY = " + CNPT_SSI_CCY);
DV.writeLog("*****MODULE = " + MODULE);

var sSqlCond = DV.addSQLCondition(null, "CNPT_ID", CNPT_ID);
sSqlCond = DV.addSQLCondition(sSqlCond, "NS_CCY", CNPT_SSI_CCY);
sSqlCond = DV.addSQLCondition(sSqlCond, "PROD_CD", MODULE);
