        ...
        <xsl:element name="root">
            <xsl:element name="domData">
                ...
                <xsl:element name="FA_BUYER_EDI_ID">
                    <xsl:value-of select="/MSG11/PmtDetails/BuyerNr"/>
                </xsl:element>
                <xsl:element name="FA_BUYER_ID"/>
                <xsl:element name="FA_BUYER_NM">
                    <xsl:value-of select="/MSG11/PmtDetails/BuyerName"/>
                </xsl:element>
                <xsl:element name="FA_EF_ID">
                    <xsl:value-of select="/MSG11/EF/FactorCode"/>
                </xsl:element>
                <xsl:element name="FA_EF_NM">
                    <xsl:value-of select="/MSG11/EF/FactorName"/>
                </xsl:element>
                <xsl:element name="FA_IF_ID">
                    <xsl:value-of select="/MSG11/IF/FactorCode"/>
                </xsl:element>
                <xsl:element name="FA_IF_NM">
                    <xsl:value-of select="/MSG11/IF/FactorName"/>
                </xsl:element>
                <xsl:element name="FA_MSG_TEXT02">
                    <xsl:value-of select="/MSG11/MsgText"/>
                </xsl:element>
                <xsl:element name="FA_PMT_AMT_SUM">
                    <xsl:value-of select="/MSG11/ControlTot/TotAmtPaid"/>
                </xsl:element>
                <xsl:element name="FA_PMT_CCY">
                    <xsl:value-of select="/MSG11/PmtBatchCurrency"/>
                </xsl:element>
                <xsl:element name="FA_PMT_DT">
                    <xsl:value-of select="/MSG11/PmtBatchDate"/>
                </xsl:element>
                <xsl:element name="FA_PMT_REF"/>
                <xsl:element name="FA_PMT_TYPE"/>
                <xsl:element name="FA_SEL_EDI_ID">
                    <xsl:value-of select="/MSG11/Seller/SellerNr"/>
                </xsl:element>
                <xsl:element name="FA_SEL_ID"/>
                <xsl:element name="FA_SEL_NM">
                    <xsl:value-of select="/MSG11/Seller/SellerName"/>
                </xsl:element>
                <xsl:element name="FA_TTL_AMT_CLEARED">
                    <xsl:value-of select="/MSG11/ControlTot/TotAmtCleared"/>
                </xsl:element>
                <xsl:element name="FA_TTL_AMT_DEDUCT">
                    <xsl:value-of select="/MSG11/ControlTot/TotAmtBankChargeDeduc"/>
                </xsl:element>
                <xsl:element name="THEIR_REF_NO">
                    <xsl:value-of select="/MSG11/PmtBatchNr"/>
                </xsl:element>
                <xsl:element name="TRX_DT"/>
                <xsl:apply-templates mode="m0" select="/MSG11/PmtDetails"/>
            </xsl:element>
        </xsl:element>
    </xsl:template>
    <xsl:template match="/MSG11/PmtDetails" mode="m0">
        <PaymentReg>
