    ...
    <xsl:template match="/">
        <xsl:element name="MSG09">
            ...
            <xsl:element name="TotAmtInvoices">
                <xsl:value-of select="/root/domData/FA_TTL_INV_AMT"/>
            </xsl:element>
            <xsl:element name="TotAmtCreditNotes">0</xsl:element>
            ...
            <xsl:element name="InvCreditNoteDetails">
                ...
                <xsl:element name="DocNr">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_DOC_NO"/>
                </xsl:element>
                <xsl:element name="DocDate">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_DOC_DT"/>
                </xsl:element>
                <xsl:element name="DocAmt">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_DOC_AMT"/>
                </xsl:element>
                <xsl:element name="DocDueDate">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_DOC_DUE_DT"/>
                </xsl:element>
                <xsl:element name="DocValueDate">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_DOC_VAL_DT"/>
                </xsl:element>
                <xsl:element name="NetPmtTerms">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_PMT_TERMS"/>
                </xsl:element>
                <xsl:element name="Discount1Days">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_PRM_DISC_DAYS"/>
                </xsl:element>
                <xsl:element name="Discount1Perc">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_PRM_DISC_RT"/>
                </xsl:element>
                <xsl:element name="Discount2Days">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_SND_DISC_DAYS"/>
                </xsl:element>
                <xsl:element name="Discount2Perc">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_SND_DISC_RT"/>
                </xsl:element>
                <xsl:element name="PmtCondition">
                    <xsl:value-of select="ee:getInterpreter(FA_PMT_COND)"/>
                </xsl:element>
                <xsl:element name="OrderNrRef">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_ORDER_NO"/>
                </xsl:element>
                <xsl:element name="InvRefNr">
                    <xsl:value-of select="/root/domData/InvTRF1/FA_INV_LINK_REF"/>
                </xsl:element>
            </xsl:element>
            <xsl:element name="ControlTot">
                <xsl:element name="TotNrInvoices">
                    <xsl:value-of select="/root/domData/FA_TTL_INV_NO"/>
                </xsl:element>
                <xsl:element name="TotNrCreditNotes">0</xsl:element>
            </xsl:element>
