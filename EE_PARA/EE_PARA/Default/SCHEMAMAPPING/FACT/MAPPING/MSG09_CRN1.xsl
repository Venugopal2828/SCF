    ...
    <xsl:template match="/">
        <xsl:element name="MSG09">
            ...
            <xsl:element name="InvBatchCurrency">
                <xsl:value-of select="/root/domData/CreditNote1/FA_DOC_CCY"/>
            </xsl:element>
            ...
            <xsl:element name="InvCreditNoteDetails">
                ...
                <xsl:element name="DocNr">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_DOC_NO"/>
                </xsl:element>
                <xsl:element name="DocDate">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_DOC_DT"/>
                </xsl:element>
                <xsl:element name="DocAmt">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_DOC_AMT"/>
                </xsl:element>
                <xsl:element name="DocDueDate">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_DOC_DUE_DT"/>
                </xsl:element>
                <xsl:element name="DocValueDate">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_DOC_VAL_DT"/>
                </xsl:element>
                <xsl:element name="NetPmtTerms">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_PMT_TERMS"/>
                </xsl:element>
                <xsl:element name="Discount1Days">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_PRM_DISC_DAYS"/>
                </xsl:element>
                <xsl:element name="Discount1Perc">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_PRM_DISC_RT"/>
                </xsl:element>
                <xsl:element name="Discount2Days">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_SND_DISC_DAYS"/>
                </xsl:element>
                <xsl:element name="Discount2Perc">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_SND_DISC_RT"/>
                </xsl:element>
                <xsl:element name="PmtCondition">
                    <xsl:value-of select="ee:getInterpreter(FA_PMT_COND)"/>
                </xsl:element>
                <xsl:element name="OrderNrRef">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_ORDER_NO"/>
                </xsl:element>
                <xsl:element name="InvRefNr">
                    <xsl:value-of select="/root/domData/CreditNote1/FA_INV_LINK_REF"/>
                </xsl:element>
            </xsl:element>
            ...
            <xsl:element name="MsgText">
                <xsl:value-of select="/root/domData/CreditNote1/FA_MSG_TEXT"/>
            </xsl:element>
        </xsl:element>
    </xsl:template>
</xsl:stylesheet>
