...
public class TokenValidationFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (uri.endsWith(".js") || uri.endsWith(".css") || uri.endsWith(".png") || uri.endsWith(".jpg")
				|| uri.endsWith(".gif") || uri.contains("/SYS_JS/") || uri.contains("/theme/")
...
//UPDATE  EJECT REASON  DATA BASE WITH REFNUMBER 
		...
		req.getSession().setAttribute("aggrNo", result.get("aggrNo"));

		saveAggrNoOnLinkOpen(cMainRef, result.get("counterpartyId"), result.get("aggrNo"));

		chain.doFilter(request, response);
		return;
	}

	private void saveAggrNoOnLinkOpen(String cMainRef, String counterpartyId, String aggrNo) {
		if (aggrNo == null || aggrNo.trim().isEmpty())
			return;
		try {
			CounterPartyDB db = new CounterPartyDB();
			String value = aggrNo.trim();
			if (cMainRef != null && !cMainRef.trim().isEmpty())
				db.updateAggrNo(cMainRef.trim(), value);
			if (counterpartyId != null && !counterpartyId.trim().isEmpty()
					&& !counterpartyId.trim().equals(cMainRef))
				db.updateAggrNo(counterpartyId.trim(), value);
			if (!value.equals(cMainRef) && (counterpartyId == null || !value.equals(counterpartyId)))
				db.updateAggrNo(value, value);
			System.out.println("[AGGR_NO] Saved on link open | aggrNo=" + value);
		} catch (Exception e) {
			System.out.println("[AGGR_NO] Save on link open failed: " + e.getMessage());
			e.printStackTrace();
		}
	}
	...
	private void redirectWithError(HttpServletRequest req, HttpServletResponse resp, String error) throws IOException {
		...
		String url = req.getContextPath() + "/counterparty_error.html?error=" + error;
